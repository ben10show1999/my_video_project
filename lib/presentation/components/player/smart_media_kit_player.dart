...
class SmartMediaKitPlayer extends StatefulWidget {
  ...
  const SmartMediaKitPlayer({super.key, required this.videoId, required this.videoTitle, required this.sources, this.thumbnailUrl, required this.onAspectRatioChanged, this.isFullscreen = false, required this.onFullscreenToggle});
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) { _initializeSmartStream(); _injectNativePreloader(); });
    Connectivity().onConnectivityChanged.listen((result) { bool hasNet = !result.contains(ConnectivityResult.none); if (_isError && hasNet && !_isCoolingDown && !_isRetrying) {
      _retryPlayback();
    ...
    player.stream.error.listen((e) { 
      ...
      // ✅ Task 7: Report Broken Link via EmailJS silently
      EmailJSTelemetry.reportBrokenLink(videoTitle: widget.videoTitle, videoId: widget.videoId, quality: _currentQualityLabel, brokenUrl: _activeUrl);
      _triggerFailover(); 
    });
    ...
    player.stream.playing.listen((p) { if (p && _thumb && mounted) setState(() => _thumb = false); if (mounted) setState(() => _isPlayingOptimistic = p); if (p) { _isSwitchingSource = false; _startActiveTimer(); _startControlsTimer(); } else {
      _pauseActiveTimer();
    } });
    player.stream.volume.listen((v) { if(mounted) setState(() => _isMutedOptimistic = v == 0); });
  }

  void _injectNativePreloader() { try { final videos = web.document.querySelectorAll('video'); for (int i = 0; i < videos.length; i++) { final v = videos.item(i) as web.HTMLVideoElement; v.setAttribute('preload', 'auto'); } } catch (_) {} }
  ...
  void _triggerFailover() { if (_isCoolingDown) return; if (_currentLinkIndex + 1 < _currentUrlList.length) { _currentLinkIndex++; ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(Provider.of<AppProvider>(context, listen: false).tr('link_fail')))); _playUrl(_currentUrlList[_currentLinkIndex], keepPosition: true); } else { player.stop(); setState(() { _isError = true; _isCoolingDown = true; }); Future.delayed(const Duration(seconds: 5), () { if (mounted) setState(() => _isCoolingDown = false); }); } }
  ...
  @override void dispose() { AiSubtitleEngine.stop(); player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); super.dispose(); }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    // ✅ Task 6: Responsive Adaptive Overlay for Subtitles
    Positioned.fill(
          ...
          return Column(
            ...
            children: [
              Container(
                ...
                padding: EdgeInsets.symmetric(horizontal: cs.maxWidth * 0.05, vertical: cs.maxHeight * 0.05),
                child: Text(text, textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: max(14, cs.maxWidth * 0.035), fontWeight: FontWeight.bold, shadows: const [Shadow(color: Colors.black, offset: Offset(1, 1), blurRadius: 2)])),
              ),
            ],
          );
        }
    ...
    Positioned(top: 16, left: 16, child: SafeArea(child: Image.asset('assets/images/Club2.webp', width: cs.maxWidth < 600 ? 40 : 65, height: cs.maxWidth < 600 ? 40 : 65, errorBuilder: (c,e,s) => const SizedBox.shrink()))),
  ])); }); }
