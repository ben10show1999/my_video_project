...
""";
  ...
  @override void initState() {
    ...
    player.stream.playing.listen((p) { 
      ...
      if (mounted) setState(() => _isPlayingOptimistic = p); // Sync truth
  ...
  // Task 6: Dynamic Playlist Swapping (Hot-Swap)
  @override
  void didUpdateWidget(SmartMediaKitPlayer oldWidget) {
    super.didUpdateWidget(oldWidget);
    // If sources changed, reset and play new video
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return AspectRatio(aspectRatio: _currentRatio, child: LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _userInteraction(), child: GestureDetector(onTap: () { 
      // Task 5: Toggle Controls on Tap
      _userInteraction(); 
      setState(() {
        if (_menuState != SettingsMenuState.none) { _menuState = SettingsMenuState.none; }
        else { _showControls = !_showControls; }
      }); 
  ...
  Positioned(
    ...
    child: SafeArea(
      child: Image.asset(
        ...
        width: cs.maxWidth < 600 ? 40 : 65, // Responsive scaling
