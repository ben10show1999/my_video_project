...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    ...
    // Auto-Recovery on Player Error (Task 28)
  ...
  // --------------------------------------------------------------------------
  // Tasks 26 & 29: ABR & Link Racing Initialization
  // --------------------------------------------------------------------------
  Future<void> _initStream() async {
    ...
    await player.open(Media(_activeUrl, httpHeaders: {'Cache-Control': 'max-age=3600', 'Range': 'bytes=0-'}), play: true);
  ...
  Future<void> _performSeamlessSwap(VideoQualityModel newQuality, String newUrl, {bool updateUIKey = false}) async {
    ...
    await player.open(Media(newUrl, httpHeaders: {'Cache-Control': 'max-age=3600'}), play: true);
  ...
  void _startAbrEngine() {
    _abrTimer = Timer.periodic(const Duration(seconds: 4), (_) async {
      ...
      if (_currentQualityKey == "Auto") {
         ...
         _handleHorizontalRacing(); // Manual Lock, Race horizontally only
      }
    });
  }
  ...
  void _triggerFailover() {
    ...
    _handleHorizontalRacing(); // Trigger immediate Failover Race
  }
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    ...
    // Task 24: Dynamic Subtitle Generation
