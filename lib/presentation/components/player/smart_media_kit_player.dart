...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    super.initState();
    // Task 33: Aggressive Continuous Preload Engine (Native Config)
    // relying on bufferSize which acts as the read-ahead buffer natively in media_kit
    player = Player(configuration: const PlayerConfiguration(bufferSize: 1024 * 1024 * 512)); // 512MB Buffer
    ...
    player.stream.duration.listen((dur) {
       if (dur.inMilliseconds == 0 || dur.inSeconds > 86400) { if (!_isLive && mounted) setState(() => _isLive = true); } 
       else { if (_isLive && mounted) setState(() => _isLive = false); }
    });

    player.stream.position.listen((pos) { VttSubtitleEngine.updatePosition(pos); });

    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await _checkSavedPosition(); _initStream(); _injectNativePreloader(); _startAutoSaveTimer(); _startSmartPulseMonitor(); _startAbrEngine();
    });
  ...
  Future<void> _resumeFromPrompt() async {
    if (_savedPosition.inSeconds > 0) { setState(() => _showResumePrompt = false); await player.seek(_savedPosition); player.play(); } else { setState(() => _showResumePrompt = false); }
  }
  ...
  void _resetAndPlayNewSource() async { 
    _isSwitchingSource = true; player.stop(); 
  ...
  Future<void> _performSeamlessSwap(VideoQualityModel newQuality, String newUrl, {bool updateUIKey = false}) async {
    ...
    if (mounted) {
      setState(() {
        _activeUrl = newUrl; _activeQualityModel = newQuality;
    ...
    if (pos.inSeconds > 0) { Future.delayed(const Duration(milliseconds: 500), () { player.seek(pos); _lastKnownNetworkPosition = Duration.zero; }); }
  ...
  void _startAbrEngine() {
    _abrTimer = Timer.periodic(const Duration(seconds: 4), (_) async {
      ...
      if (_currentQualityKey == "Auto") {
         if (isBuffering) {
            _bufferingTicks++;
            if (_bufferingTicks >= 2) { _dropQualityLevel(); }
         } else {
            _bufferingTicks = 0; _handleHorizontalRacing();
         }
      } else {
  ...
  void _toggleFullscreenLogic() {
    try {
      if (!widget.isFullscreen) { web.document.documentElement?.requestFullscreen(); if (_currentRatio > 1.0) { web.window.screen.orientation.lock('landscape'); } else { web.window.screen.orientation.lock('portrait'); } } 
      else { web.document.exitFullscreen(); web.window.screen.orientation.unlock(); }
    } catch (e) { /* Ignore */ }
  ...
  @override void dispose() { 
    ...
    _savePositionTimer?.cancel(); _pulseTimer?.cancel(); VttSubtitleEngine.stop(); 
    player.dispose(); 
    _abrTimer?.cancel(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); 
    super.dispose(); 
  }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    if (RemoteConfigService.appLogoUrl != null && RemoteConfigService.appLogoUrl!.isNotEmpty)
      Positioned(top: 16, left: 16, child: SafeArea(child: SizedBox(width: (cs.maxWidth * 0.06).clamp(30.0, 70.0), child: UniversalImage(path: RemoteConfigService.appLogoUrl!, fit: BoxFit.contain)))),
  ...
  Widget _buildControlsUI(AppProvider p) => Positioned(bottom: 0, left: 0, right: 0, child: Container(color: Colors.black54, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: SafeArea(top: false, child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    Row(children: [
      ...
      if (!_isLive) IconButton(icon: Icon(_loop ? Icons.repeat_one : Icons.repeat, color: _loop ? Colors.amber : Colors.white), onPressed: () => setState(() { _loop = !_loop; player.setPlaylistMode(_loop ? PlaylistMode.single : PlaylistMode.none); })), 
      if (_isLive) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.red, borderRadius: BorderRadius.circular(4)), child: const Text("LIVE", style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold))),
      const Spacer(), 
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    ...
    if (_menuState == SettingsMenuState.subtitles) { 
      return [
        ...
        ...widget.subtitles.keys.map((k) => ListTile(
          dense: true, title: Text(k.toUpperCase(), style: const TextStyle(color: Colors.white)),
          trailing: _currentSubtitleKey == k ? const Icon(Icons.check, color: Colors.amber, size: 16) : null,
          onTap: () { setState(() { _currentSubtitleKey = k; VttSubtitleEngine.loadSubtitles(k, widget.subtitles); }); }
        ))
      ]; 
    }
    return [];
