...
import 'package:my_video_project/core/utils/vtt_engine.dart';
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    ...
    // ✅ Task 4: Sync VTT Engine with player position
    player.stream.position.listen((pos) {
      if (!mounted) return;
      VttEngine.updatePosition(pos);
    });
  ...
  Future<void> _initializeSmartStream() async {
    ...
    VttEngine.setLanguage(_currentSubtitleLang); // Initialize Subtitles
    if (_isAutoMode) {
  ...
  @override void dispose() { 
    ...
    _pulseTimer?.cancel();
    VttEngine.stop(); player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); super.dispose(); 
  }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    // ✅ Task 5: RepaintBoundary around Video
    RepaintBoundary(
      child: _isBlocked 
    ...
    // ✅ Tasks 4 & 5: RepaintBoundary around Absolute Adaptive Subtitle Overlay (VTT)
    RepaintBoundary(
          ...
          child: Align(
            ...
            child: Padding(
              ...
              child: ValueListenableBuilder<String>(
                valueListenable: VttEngine.currentSubtitle,
                builder: (context, text, child) {
                  if (text.isEmpty || _currentSubtitleLang == "Off") return const SizedBox.shrink();
    ...
    // ✅ Task 2: Smart Pulse Loader
    if (player.state.buffering && !_isActuallyAdvancing && !_isError) const Center(child: CircularProgressIndicator(color: Colors.amber)),
    
    // ✅ Task 4: VTT Loading Indicator
    ValueListenableBuilder<bool>(
      valueListenable: VttEngine.isLoading,
      builder: (context, isLoading, child) {
        if (!isLoading) return const SizedBox.shrink();
        return Positioned(top: 16, right: 16, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(4)), child: Row(children: [const SizedBox(width: 12, height: 12, child: CircularProgressIndicator(color: Colors.amber, strokeWidth: 2)), const SizedBox(width: 8), Text(p.tr('sub_loading'), style: const TextStyle(color: Colors.amber, fontSize: 10))])));
      }
    ),

    if (!_isAd && !_isIntervalAd && !_isBlocked && !_isError && _showControls) RepaintBoundary(child: _buildControlsUI(p)), 
    if (!_isAd && !_isIntervalAd && _menuState != SettingsMenuState.none) RepaintBoundary(child: _buildMenuUI(cs.biggest, p)), 
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    ...
    if (_menuState == SettingsMenuState.subtitles) {
      return [
      ListTile(dense: true, title: Text(p.tr('back'), style: const TextStyle(color: Colors.amber)), onTap: () => setState(() => _menuState = SettingsMenuState.main)), 
      ListTile(dense: true, title: Text(p.tr('disable'), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == "Off" ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = "Off"; VttEngine.setLanguage("Off"); }); }),
      ..._availableLangs.map((l) => ListTile(dense: true, title: Text(l == "Auto" ? p.tr('auto') : p.tr(l.toLowerCase()), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == l ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = l; VttEngine.setLanguage(l); }); }))
    ];
    }
    return [];
  }
