...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  bool _isRetrying = false; // TASK 1: Lock for retry storm
...
""";
  ...
  void _resetAndPlayNewSource() async {
    player.stop(); 
    setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isRetrying = false; _isPlayingOptimistic = true; });
  ...
  // TASK 6: Instant Global Tap-to-Hide
  ...
  // TASK 7: Smart Content-Aware Fullscreen
  void _toggleFullscreen() async { 
    if (_isFullscreen) { 
      ...
      if (_currentRatio > 1.05) {
         // Landscape (Wide)
         await SystemChrome.setPreferredOrientations([DeviceOrientation.landscapeLeft, DeviceOrientation.landscapeRight]); 
      } else {
         // Portrait (Vertical / Reels) -> Stay vertical but immersive
         await SystemChrome.setPreferredOrientations([DeviceOrientation.portraitUp]); 
      }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return AspectRatio(aspectRatio: _currentRatio, child: LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    // LAYER 1: VIDEO (Smart BoxFit for Immersive)
    _isBlocked ? Container(color: Colors.black, width: double.infinity, height: double.infinity, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.lock_clock, color: Colors.white54, size: 50), const SizedBox(height: 16), Text(p.tr('protected'), style: const TextStyle(color: Colors.white, fontSize: 18)), Text(p.tr('wait_msg'), style: const TextStyle(color: Colors.white54))])) 
               : Video(controller: controller, controls: NoVideoControls, fit: (_isFullscreen && _currentRatio < 1.05) ? BoxFit.cover : BoxFit.contain), 
    ...
    // LAYER 2: INSTANT TAP-TO-HIDE SHIELD (TASK 6)
    Positioned.fill(child: GestureDetector(
      behavior: HitTestBehavior.opaque, // Must catch taps before they hit the video
    ...
    // LAYER 6: FLUID AD CONTAINER
    if (_isAd || _isIntervalAd) Positioned.fill(child: WebAdBanner(adScript: _adScriptCode, onClose: _closeAd, isInternal: false, isInteractive: false)),
