...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    ...
    // Task 7: Ghost Error Filter (Intercept Abort/Interrupted)
    player.stream.error.listen((e) { 
      final errStr = e.toString().toLowerCase();
      if (_isSwitchingSource || errStr.contains('abort') || errStr.contains('interrupt')) return; // IGNORING FALSE ALARMS
      _triggerFailover(); 
    });
  ...
  // Task 6: Native Web Fullscreen with Orientation Lock
  void _toggleFullscreen() { 
    ...
    Navigator.push(context, MaterialPageRoute(builder: (_) => FullscreenVideoPage(
      ...
      ratio: _currentRatio,
      onExit: () {
        try { web.document.exitFullscreen(); web.window.screen.orientation.unlock(); } catch (e) { /* Ignore unsupported API errors silently */ }
      }
    )));
  }
