...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  bool _isRetrying = false; // The Logic Lock for Retry Storms
  ...
  double _currentRatio = 16/9; // Track Aspect Ratio for Smart Fullscreen
...
""";
  ...
  @override void initState() {
    ...
    // Listen to Dimensions for Context-Aware Fullscreen
    ...
    // Connectivity Listener for Anti-Freeze & Hard Reload
    Connectivity().onConnectivityChanged.listen((result) { 
      bool hasNet = !result.contains(ConnectivityResult.none);
      if (_isError && hasNet && !_isCoolingDown && !_isRetrying) {
         _retryPlayback(); // Trigger Hard Reload
      } else if (!hasNet) {
         if (player.state.playing) player.stop(); // Kill engine
  ...
  void _resetAndPlayNewSource() async {
    player.stop(); 
    setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isPlayingOptimistic = true; });
  ...
  // TASK 5: CONTEXT-AWARE FULLSCREEN
  ...
  // ✅ FIX: _closeAd is now properly passed to WebAdBanner
  void _closeAd() { if (mounted) { setState(() { _isAd = false; _isIntervalAd = false; }); player.play(); } }
  
  @override void dispose() { player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); super.dispose(); }

  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return AspectRatio(aspectRatio: _currentRatio, child: LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    
    // 1. VIDEO LAYER (Immersive expansion based on orientation)
    _isBlocked ? Container(color: Colors.black, width: double.infinity, height: double.infinity, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.lock_clock, color: Colors.white54, size: 50), const SizedBox(height: 16), Text(p.tr('protected'), style: const TextStyle(color: Colors.white, fontSize: 18)), Text(p.tr('wait_msg'), style: const TextStyle(color: Colors.white54))])) 
               : Video(controller: controller, controls: NoVideoControls, fit: (_isFullscreen && _currentRatio < 1.0) ? BoxFit.cover : BoxFit.contain), 
    ...
    // 6. AD CONTAINER (Fluid width, inline constraint) // ✅ FIX: Added onClose
    if (_isAd || _isIntervalAd) Positioned.fill(child: WebAdBanner(adScript: _adScriptCode, heightConstraint: 250, onClose: _closeAd)),
  ...
  Widget _controls() => Positioned(bottom: 0, left: 0, right: 0, child: Container(color: Colors.black54, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: SafeArea(top: false, child: Column(mainAxisSize: MainAxisSize.min, children: [
    SliderTheme(data: SliderThemeData(trackHeight: 2, thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 6)), child: StreamBuilder<Duration>(stream: player.stream.position, builder: (c, s) { 
        ...
        return Row(children: [Text(_fmt(Duration(seconds: currentVal.toInt())), style: const TextStyle(color: Colors.white, fontSize: 12)), Expanded(child: Slider(
          ...
          onChangeEnd: (v) { player.seek(Duration(seconds: v.toInt())); setState(() => _draggingValue = null); }
