...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  // TASK 1: Player Anti-Freeze Timer
  ...
  // TASK 4: Video Dimensions for Context-Aware Fullscreen
  ...
  // TASK 3: Optimistic UI & Decoupled Slider
  bool _isPlayingOptimistic = false;
  bool _isMutedOptimistic = false;
  double? _draggingValue; // Determines if user is dragging slider

  final String _adScriptCode = """
<script>atOptions = { 'key' : 'ea534947875b8853a56110f9767a6a83', 'format' : 'iframe', 'height' : 250, 'width' : 300, 'params' : {} };</script>
...
""";
  ...
  @override void initState() {
    ...
    player.stream.videoParams.listen((params) { 
      ...
      if (w != null && h != null && h > 0) { 
        ...
        _videoHeight = h.toDouble();
        double nr = _videoWidth / _videoHeight; 
    ...
    Connectivity().onConnectivityChanged.listen((result) { 
      if (_isError && !result.contains(ConnectivityResult.none)) {
        if (!_isRetrying) _triggerFailover(); 
      }
    });
  ...
  void _resetAndPlayNewSource() async {
    player.stop();
    setState(() { _isError = false; _thumb = true; _isRetrying = false; _activeUrl = ""; });
  ...
  // TASK 3: Instant Tap-to-Hide Logic
  ...
  void _triggerFailover() {
    ...
    // TASK 1: Anti-Freeze Implementation (5s Delay)
    if (_retryCoolDownTimer != null && _retryCoolDownTimer!.isActive) return;
    
    _retryCoolDownTimer = Timer(const Duration(seconds: 5), () {
  ...
  // TASK 4: Smart Context-Aware Fullscreen Logic
  void _toggleFullscreen() async { 
    if (_isFullscreen) { 
      ...
      // Enter Logic based on Video Aspect Ratio
      await SystemChrome.setEnabledSystemUIMode(SystemUiMode.immersiveSticky); 
      
      bool isLandscapeVideo = _videoWidth >= _videoHeight; // e.g., 16:9
      
      if (isLandscapeVideo) {
         // Force Landscape for Wide Videos
         await SystemChrome.setPreferredOrientations([DeviceOrientation.landscapeLeft, DeviceOrientation.landscapeRight]); 
      } else {
         // Force Portrait for Shorts/Vertical (Immersive)
         await SystemChrome.setPreferredOrientations([DeviceOrientation.portraitUp]);
      }
  ...
  @override void dispose() { player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); _retryCoolDownTimer?.cancel(); super.dispose(); }

  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return AspectRatio(aspectRatio: _currentRatio, child: LayoutBuilder(builder: (c, cs) { 
    return Container(color: Colors.black, child: Stack(alignment: Alignment.center, children: [
      ...
      // TASK 3: Invisible "Tap-to-Hide" Gesture Layer
      if (!_isAd && !_isIntervalAd && !_isBlocked)
        Positioned.fill(
      ...
      // 6. Error Layer
      if (_isError) Positioned.fill(child: Container(color: Colors.black87, child: Center(child: FittedBox(fit: BoxFit.scaleDown, child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.wifi_off, color: Colors.red, size: 50), const SizedBox(height: 16), Text(p.tr('connection_error'), style: const TextStyle(color: Colors.white, fontSize: 16)), const SizedBox(height: 16), ElevatedButton.icon(icon: const Icon(Icons.refresh), label: Text(p.tr('retry')), style: ElevatedButton.styleFrom(backgroundColor: Colors.amber, foregroundColor: Colors.black), onPressed: _triggerFailover)]))))), 
      
      // 7. Buffering
      if (player.state.buffering || _isRetrying) const Center(child: CircularProgressIndicator(color: Colors.amber)),

      // TASK 2: Ad Layer (Elastic)
      if (_isAd || _isIntervalAd) Positioned.fill(child: WebAdBanner(adScript: _adScriptCode, width: double.infinity, height: double.infinity, onClose: _closeAd)),
  ...
  // TASK 3: Decoupled Slider Logic (Zero Latency)
  Widget _controls() => Positioned(bottom: 0, left: 0, right: 0, child: Container(color: Colors.black54, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: SafeArea(top: false, child: Column(mainAxisSize: MainAxisSize.min, children: [
    SliderTheme(data: SliderThemeData(trackHeight: 2, thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 6)), child: StreamBuilder<Duration>(stream: player.stream.position, builder: (c, s) { 
        ...
        // Use local dragging value if valid, else actual position
        ...
        return Row(children: [Text(_fmt(Duration(seconds: currentVal.toInt())), style: const TextStyle(color: Colors.white, fontSize: 12)), Expanded(child: Slider(
          ...
          onChangeStart: (v) { setState(() => _draggingValue = v); _controlsTimer?.cancel(); }, // Pause hiding
          onChanged: (v) { setState(() => _draggingValue = v); }, // Instant visual update
          onChangeEnd: (v) { 
             player.seek(Duration(seconds: v.toInt())); // Seek only on release
    ...
    Row(children: [
      IconButton(
        ...
        onPressed: () { setState(() => _isPlayingOptimistic = !_isPlayingOptimistic); player.playOrPause(); }
      ), 
      IconButton(
        icon: Icon(_isMutedOptimistic ? Icons.volume_off : Icons.volume_up, color: Colors.white), 
        onPressed: () { setState(() => _isMutedOptimistic = !_isMutedOptimistic); player.setVolume(player.state.volume == 0 ? 100 : 0); }
      ),
