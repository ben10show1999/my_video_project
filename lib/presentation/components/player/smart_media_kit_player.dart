...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  // Fields now used in Subtitles menu
  bool _ai = false; 
  bool _blackScreen = false; bool _isError = false; bool _isIntervalAd = false; bool _isFullscreen = false;
  Timer? _recordingTimer; Timer? _intervalAdTimer;
  // Field now used in Subtitles menu
  String _currentAiLang = "English"; 
  String _currentQualityLabel = "Auto"; String _activeUrl = ""; bool _isAutoMode = true; double _currentRatio = 16/9;
  ...
  @override void initState() {
    ...
    _intervalAdTimer = Timer.periodic(const Duration(minutes: 30), (timer) { if (player.state.playing && !_isIntervalAd && !_isAd) _triggerIntervalAd(); });
    ...
    player.stream.playing.listen((p) { if (p && _thumb && mounted) setState(() => _thumb = false); if (p) {
