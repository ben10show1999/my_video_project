...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  // Task 12: HLS Support properties
  ...
  // ✅ Task 15: Player Lifecycle Fix - Watch the videoId instead of sources array
  ...
  void _resetAndPlayNewSource() async { 
    ...
    player.stop(); 
    setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isPlayingOptimistic = true; _showResumePrompt = false; _isHlsStream = false; }); 
    await _initializeSmartStream(); 
  }

  void _startControlsTimer() { _controlsTimer?.cancel(); if (mounted) { setState(() => _showControls = true); } _controlsTimer = Timer(const Duration(seconds: 3), () { if (mounted && (player.state.playing || _isHlsStream) && _menuState == SettingsMenuState.none) { setState(() => _showControls = false); } }); }
  ...
  // ✅ Task 12: Universal Player Engine (HLS Support)
  void _playUrl(String url, {bool keepPosition = false}) { 
    ...
    setState(() { _isHlsStream = false; _isError = false; });
  ...
  void _setupHlsPlayer(String url) {
    setState(() { _isHlsStream = true; _isError = false; _thumb = false; });
    _hlsViewId = 'hls_player_${DateTime.now().millisecondsSinceEpoch}';
    ...
    final content = """
      ...
      <style> html, body { margin: 0; padding: 0; width: 100%; height: 100%; background: black; overflow: hidden; } video { width: 100%; height: 100%; object-fit: contain; } </style>
  ...
  void _handleSubtitles() { if (_currentSubtitleLang != "Off") { VttSubtitleEngine.loadSubtitles(_currentSubtitleLang, enUrl: widget.subtitleUrlEn, arUrl: widget.subtitleUrlAr); } else { VttSubtitleEngine.stop(); } }

  void _retryPlayback() { 
    if (_isCoolingDown || _isRetrying) { return; } 
    setState(() { _isError = false; _isCoolingDown = true; _isRetrying = true; });
    if (_isHlsStream) { _setupHlsPlayer(_activeUrl); } else { player.open(Media(_activeUrl)); }
    Future.delayed(const Duration(milliseconds: 500), () { if (!_isHlsStream && _lastKnownNetworkPosition.inSeconds > 0) { player.seek(_lastKnownNetworkPosition); _lastKnownNetworkPosition = Duration.zero; } });
  ...
  void _closeAd() { if (mounted) { setState(() { _isAd = false; _isIntervalAd = false; }); if(!_isHlsStream) player.play(); } }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    if (!_isAd && !_isIntervalAd && _thumb && (!player.state.playing && !_isHlsStream) && !_isError) Center(child: IconButton(iconSize: 64, icon: const Icon(Icons.play_circle_fill, color: Colors.white), onPressed: () => player.play())), 
    ...
    if (player.state.buffering && !_isActuallyAdvancing && !_isError && !_isHlsStream) const Center(child: CircularProgressIndicator(color: Colors.amber)),
    if (!_isAd && !_isIntervalAd && !_isBlocked && !_isError && _showControls && !_isHlsStream) _buildControlsUI(p), 
    if (!_isAd && !_isIntervalAd && _menuState != SettingsMenuState.none && !_isHlsStream) _buildMenuUI(cs.biggest, p), 
    if ((_isAd || _isIntervalAd) && widget.adScript != null) Positioned.fill(child: WebAdBanner(adScript: widget.adScript, onClose: _closeAd)),
    // ✅ Task 14: Dynamic Logo fetched from RemoteConfigService
    Positioned(top: 16, left: 16, child: SafeArea(child: SizedBox(width: (cs.maxWidth * 0.06).clamp(30.0, 70.0), child: UniversalImage(path: RemoteConfigService.appConfig?.playerLogoUrl ?? 'https://iili.io/q2Ty8N4.webp', fit: BoxFit.contain)))),
    if (_showResumePrompt && !_isHlsStream) Positioned(bottom: 24, left: 16, right: 16, child: SafeArea(child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), decoration: BoxDecoration(color: const Color(0xFF323232), borderRadius: BorderRadius.circular(8), boxShadow: const [BoxShadow(color: Colors.black45, blurRadius: 10, offset: Offset(0, 4))]), child: Directionality(textDirection: TextDirection.rtl, child: Row(children: [Expanded(child: Text(p.tr('resume_msg'), style: const TextStyle(color: Colors.white, fontSize: 14))), TextButton(onPressed: _resumeFromPrompt, child: Text(p.tr('resume_btn'), style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold))), IconButton(icon: const Icon(Icons.close, color: Colors.white70, size: 20), padding: EdgeInsets.zero, constraints: const BoxConstraints(), onPressed: () => setState(() => _showResumePrompt = false))])))))
  ])); }); }
