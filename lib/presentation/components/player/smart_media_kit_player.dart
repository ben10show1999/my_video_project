...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  bool _isCoolingDown = false; // Prevents infinite loop
  ...
  double? _draggingValue; // Decoupled Slider
...
""";
  ...
  @override void initState() {
    ...
    Connectivity().onConnectivityChanged.listen((result) { 
      ...
      if (_isError && hasNet && !_isCoolingDown) {
         ...
         // Stop player immediately to save resources
         if (player.state.playing) player.stop(); // CRITICAL: Kill engine
  ...
  void didUpdateWidget(SmartMediaKitPlayer oldWidget) {
    ...
    // HOT-SWAP LOGIC: Kill old, start new
  ...
  void _triggerFailover() {
    ...
    if (_currentLinkIndex + 1 < _currentUrlList.length) {
      ...
      // ANTI-FREEZE: Trigger Cool-down
      player.stop(); // Stop engine
  ...
  void _retryPlayback() { 
    if (_isCoolingDown || _isRetrying) return; // Prevent Storm
    setState(() { _isError = false; _isCoolingDown = true; _isRetrying = true; });
    
    // HARD RELOAD LOGIC
    player.open(Media(_activeUrl)); // Re-open socket, don't just 'play()'
  ...
  // SMART FULLSCREEN (Content-Aware)
  void _toggleFullscreen() async { 
    if (_isFullscreen) { 
      ...
      if (_currentRatio >= 1) {
         // Landscape (Wide Video) -> Force Rotate
         await SystemChrome.setPreferredOrientations([DeviceOrientation.landscapeLeft]); 
      } else {
         // Portrait (Vertical Video) -> Stay Portrait + Immersive
         await SystemChrome.setPreferredOrientations([DeviceOrientation.portraitUp]); 
      }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return AspectRatio(aspectRatio: _currentRatio, child: LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    _isBlocked ? Container(color: Colors.black, width: double.infinity, height: double.infinity, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.lock_clock, color: Colors.white54, size: 50), const SizedBox(height: 16), Text(p.tr('protected'), style: const TextStyle(color: Colors.white, fontSize: 18)), Text(p.tr('wait_msg'), style: const TextStyle(color: Colors.white54))])) 
               : Video(controller: controller, controls: NoVideoControls, fit: (_isFullscreen && _currentRatio < 1) ? BoxFit.cover : BoxFit.contain), 
    ...
    // 2. TAP-TO-HIDE LAYER (Transparent GestureDetector)
