...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    ...
    player.stream.position.listen((pos) {
      VttSubtitleEngine.updatePosition(pos); // ✅ Fixed: Removed unnecessary != null
    });
    ...
    Connectivity().onConnectivityChanged.listen((result) { 
      ...
      if (_isError && hasNet && !_isCoolingDown && !_isRetrying) { _retryPlayback(); } // ✅ Fixed
      else if (!hasNet) { 
  ...
  Future<void> _initializeSmartStream() async {
    ...
    if (_isAutoMode) { await _autoSelectBestQualityAndLink(); } else { await _manualSelectQuality(pref); } // ✅ Fixed
  }
  ...
  void _handleSubtitles() { if (_currentSubtitleLang != "Off") { VttSubtitleEngine.loadSubtitles(_currentSubtitleLang); } else { VttSubtitleEngine.stop(); } }
  ...
  void _toggleFullscreenLogic() {
    try {
      if (!widget.isFullscreen) {
        ...
        if (_currentRatio > 1.0) { web.window.screen.orientation.lock('landscape'); } else { web.window.screen.orientation.lock('portrait'); } // ✅ Fixed
      } else {
