...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  // Optimistic UI State (Task 4)
  bool _isPlayingOptimistic = false;
  bool _isMutedOptimistic = false;
  double? _draggingValue; // Decoupled Slider Value
...
""";
  ...
  @override void initState() {
    ...
    player.stream.playing.listen((p) { 
      ...
      if (mounted) setState(() => _isPlayingOptimistic = p); // Sync optimistic state
  ...
  // Task 6: Dynamic Playlist Hot-Swap
  @override
  ...
  void _resetAndPlayNewSource() async {
    ...
    setState(() {
      ...
      _thumb = true;
      _isRetrying = false;
      _isPlayingOptimistic = true; // Assume play on swap
    });
    // Restart race
  ...
  // Task 4: Tap-to-Hide/Show Logic
  void _handleScreenTap() {
     if (_showControls && _menuState == SettingsMenuState.none) {
       // If showing and no menu, hide immediately
       setState(() => _showControls = false);
       _controlsTimer?.cancel();
     } else {
       // Show controls
       setState(() {
         _showControls = true;
         _menuState = SettingsMenuState.none; // Reset menu
       });
       _startControlsTimer();
     }
  }
  ...
  // Task 1: Retry with Cool-down
  void _retryPlayback() { 
    if (_isRetrying) return;
    _isRetrying = true;
    setState(() => _isError = false);
    
    // Cool-down before actual retry logic
    Future.delayed(const Duration(seconds: 5), () {
       if (mounted) {
          _playUrl(_activeUrl, keepPosition: true);
          // Allow another few seconds before clearing retry flag
  ...
  // Task 5: Smart Context-Aware Fullscreen
  void _toggleFullscreen() async { 
    if (_isFullscreen) { 
      ...
      // Enter Fullscreen Smartly
      await SystemChrome.setEnabledSystemUIMode(SystemUiMode.immersiveSticky); 
      if (_currentRatio >= 1) {
         // Landscape (Wide)
         await SystemChrome.setPreferredOrientations([DeviceOrientation.landscapeLeft]); 
      } else {
         // Portrait (Tall) - Stay portrait but immersive
         await SystemChrome.setPreferredOrientations([DeviceOrientation.portraitUp]); 
      }
  ...
  // Task 3: Elastic Ad Container (Full Stretch)
  ...
  // Task 7: Brand Logo (Top Left, Responsive)
  Positioned(
    top: 16, left: 16,
    child: SafeArea(
      child: Image.asset(
        'assets/images/Club2.webp',
        width: cs.maxWidth < 600 ? 40 : 65,
        height: cs.maxWidth < 600 ? 40 : 65,
  ...
  // Task 4: Optimistic UI Controls (Slider & Buttons)
