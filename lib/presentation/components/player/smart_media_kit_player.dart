...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  // Point-In-Time Network Recovery
  ...
  @override void initState() {
    ...
    player.stream.position.listen((pos) {
      VttSubtitleEngine.syncPosition(pos);
    });
  ...
  void _playUrl(String url, {bool keepPosition = false}) { 
    ...
    VttSubtitleEngine.loadSubtitles(_currentSubtitleLang);
  }
  ...
  @override void dispose() { 
    ...
    _pulseTimer?.cancel();
    VttSubtitleEngine.stop(); player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); super.dispose(); 
  }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    // ✅ Task 5: RepaintBoundary around the core UI to prevent scrolling freeze
    RepaintBoundary(
      child: Stack(
        ...
        children: [
          ...
          // ✅ Task 4: Absolute Adaptive Subtitle Overlay (Zero Lag Micro-Rebuilds)
          Positioned.fill(
            child: AspectRatio(
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    ...
    if (_menuState == SettingsMenuState.subtitles) {
      return [
      ListTile(dense: true, title: Text(p.tr('back'), style: const TextStyle(color: Colors.amber)), onTap: () => setState(() => _menuState = SettingsMenuState.main)), 
      ListTile(dense: true, title: Text(p.tr('disable'), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == "Off" ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = "Off"; VttSubtitleEngine.loadSubtitles("Off"); }); }),
      ..._availableLangs.map((l) => ListTile(dense: true, title: Text(l == "Auto" ? p.tr('auto') : p.tr(l.toLowerCase()), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == l ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = l; VttSubtitleEngine.loadSubtitles(l); }); }))
    ];
    }
    return [];
  }
