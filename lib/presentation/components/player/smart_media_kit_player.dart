...
class SmartMediaKitPlayer extends StatefulWidget {
  final String videoId; final List<VideoQualityModel> sources; final String videoTitle; final bool isLive;
  final String? thumbnailUrl; final Function(double ratio) onAspectRatioChanged;
  final bool isFullscreen; final VoidCallback onFullscreenToggle;
  
  const SmartMediaKitPlayer({super.key, required this.videoId, required this.sources, required this.videoTitle, this.isLive = false, this.thumbnailUrl, required this.onAspectRatioChanged, this.isFullscreen = false, required this.onFullscreenToggle});
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  // DRM & Persistence State
  Timer? _activeTimer; Timer? _cooldownTimer; Timer? _intervalAdTimer; Timer? _trackerTimer;
  ...
  // Resume Bar State
  bool _showResumeBar = false; int _savedPosition = 0;
  ...
  @override void initState() {
    ...
    _trackerTimer = Timer.periodic(const Duration(seconds: 5), (t) async {
       if (!widget.isLive && player.state.playing && player.state.position.inSeconds > 0) {
           final prefs = await SharedPreferences.getInstance();
           prefs.setInt('pos_${widget.videoId}', player.state.position.inSeconds);
       }
    });
  }
  ...
  Future<void> _checkSavedPosition() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    int saved = prefs.getInt('pos_${widget.videoId}') ?? 0;
    if (saved > 10) { setState(() { _savedPosition = saved; _showResumeBar = true; }); }
  }
  ...
  void _injectNativePreloader() {
    ...
    } catch (_) { /* Ignore */ } // FIXED LINT WARNING
  }
  ...
  void _resetAndPlayNewSource() async { 
    ...
    player.stop(); 
    setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isPlayingOptimistic = true; _showResumeBar = false; }); 
    await _initializeSmartStream(); 
  }

  void _startControlsTimer() { _controlsTimer?.cancel(); if(mounted) setState(() => _showControls = true); _controlsTimer = Timer(const Duration(seconds: 3), () { if (mounted && player.state.playing && _menuState == SettingsMenuState.none && !_showResumeBar) setState(() => _showControls = false); }); }
  ...
  void _releaseBlock() { if (!mounted) return; setState(() => _isBlocked = false); player.play(); }
  ...
  void _playUrl(String rawUrl, {bool keepPosition = false, bool cacheBust = false}) { 
    if (rawUrl.isEmpty) return; 
    _activeUrl = rawUrl; 
    String finalUrl = rawUrl;
    
    if (cacheBust) finalUrl += '${finalUrl.contains('?') ? '&' : '?'}cb=${DateTime.now().millisecondsSinceEpoch}';
    
    String encryptedPayload = base64Encode(utf8.encode(finalUrl));
    String decoyUrl = "/protected_stream/$encryptedPayload";

    Duration c = player.state.position; setState(() => _isError = false); 
    player.open(Media(decoyUrl), play: true); 
  ...
  void _retryPlayback() { 
    ...
    _playUrl(_activeUrl, keepPosition: true, cacheBust: true); 
  ...
  void _toggleFullscreenLogic() {
    ...
    } catch (e) { /* Ignore unsupported API errors silently */ } // FIXED LINT WARNING
  ...
  @override void dispose() { 
    ...
    AiSubtitleEngine.stop(); player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); _trackerTimer?.cancel(); super.dispose(); 
  }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    if (player.state.buffering && !_isError && !_isPlayingOptimistic) const Center(child: CircularProgressIndicator(color: Colors.amber)),
    ...
    if (_showResumeBar)
      Positioned(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFF202020).withValues(alpha: 0.95), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.white10), boxShadow: const [BoxShadow(color: Colors.black54, blurRadius: 10, offset: Offset(0, 4))]),
          child: Row(
            children: [
              TextButton.icon(onPressed: () { player.seek(Duration(seconds: _savedPosition)); setState(() => _showResumeBar = false); }, icon: const Icon(Icons.play_arrow, color: Colors.black, size: 18), label: Text(p.tr('resume_btn'), style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold)), style: TextButton.styleFrom(backgroundColor: Colors.amber, padding: const EdgeInsets.symmetric(horizontal: 16))),
              const SizedBox(width: 16), Expanded(child: Text(p.tr('resume_msg'), style: const TextStyle(color: Colors.white, fontSize: 13))),
              IconButton(icon: const Icon(Icons.close, color: Colors.white54, size: 20), onPressed: () => setState(() => _showResumeBar = false))
            ]
          )
        )
      ),
