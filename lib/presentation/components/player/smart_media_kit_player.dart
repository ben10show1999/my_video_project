...
class SmartMediaKitPlayer extends StatefulWidget {
  ...
  final Map<String, String> subtitles; final String? originalLang; final String? adScript;
  const SmartMediaKitPlayer({super.key, required this.sources, required this.videoTitle, required this.videoId, this.totalDuration, this.thumbnailUrl, required this.onAspectRatioChanged, this.isFullscreen = false, required this.onFullscreenToggle, required this.subtitles, this.originalLang, this.adScript});
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    ...
    player.stream.playing.listen((p) { 
      if (p && _thumb && mounted) { setState(() { _thumb = false; _autoplayBlocked = false; }); }
  ...
  void _injectNativePreloader() {
    ...
    } catch (_) { /* Ignore preloader errors */ }
  }
  ...
  void _resetAndPlayNewSource() async { 
    ...
    player.stop(); 
    setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isPlayingOptimistic = true; _showResumePrompt = false; _autoplayBlocked = false; }); 
  ...
  void _playUrl(String url, {bool keepPosition = false}) { 
    ...
    setState(() { _isError = false; _autoplayBlocked = false; }); 
    player.open(Media(url, httpHeaders: {'Cache-Control': 'max-age=3600', 'Range': 'bytes=0-'}), play: true); 
    
    Future.delayed(const Duration(milliseconds: 2000), () {
      if (mounted && !player.state.playing && !_isError) { setState(() => _autoplayBlocked = true); }
    });
  ...
  void _handleSubtitles() { 
    if (_currentSubtitleLang != "Off") { VttSubtitleEngine.loadSubtitles(_currentSubtitleLang, widget.subtitles, widget.originalLang); } 
    else { VttSubtitleEngine.stop(); } 
  }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    if (_autoplayBlocked && !_isError) Positioned.fill(child: Container(color: Colors.black54, child: Center(child: InkWell(onTap: () { setState(() => _autoplayBlocked = false); player.play(); }, child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.play_circle_fill, color: Colors.amber, size: 80), const SizedBox(height: 8), Text(p.tr('autoplay_blocked'), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))]))))),
    
    if (!_isAd && !_isIntervalAd && _thumb && !player.state.playing && !_isError && !_autoplayBlocked) Center(child: IconButton(iconSize: 64, icon: const Icon(Icons.play_circle_fill, color: Colors.white), onPressed: () => player.play())), 
    ...
    if (player.state.buffering && !_isActuallyAdvancing && !_isError && !_isLive && !_autoplayBlocked) const Center(child: CircularProgressIndicator(color: Colors.amber)),
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    ...
    if (_menuState == SettingsMenuState.subtitles) { return [ListTile(dense: true, title: Text(p.tr('back'), style: const TextStyle(color: Colors.amber)), onTap: () => setState(() => _menuState = SettingsMenuState.main)), ListTile(dense: true, title: Text(p.tr('disable'), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == "Off" ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = "Off"; _handleSubtitles(); }); }), ListTile(dense: true, title: Text(p.tr('auto'), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == "Auto" ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = "Auto"; _handleSubtitles(); }); }), ...widget.subtitles.keys.map((l) => ListTile(dense: true, title: Text(l.toUpperCase(), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == l ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = l; _handleSubtitles(); }); }))]; }
    return [];
  }
}
