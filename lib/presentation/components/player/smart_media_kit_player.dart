...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  final List<String> _availableLangs = ["Auto", "English", "Arabic", "French"];
  
  final String _vttAr = "https://cdn.plyr.io/static/demo/thumbs/100p.vtt";
  final String _vttEn = "https://cdn.plyr.io/static/demo/thumbs/240p.vtt";
  final String _vttFr = "https://cdn.plyr.io/static/demo/View_From_A_Blue_Moon_Trailer-HD.en.vtt";
  ...
  final ValueNotifier<bool> _isPlayingOptimistic = ValueNotifier(false); 
  final ValueNotifier<bool> _isMutedOptimistic = ValueNotifier(false); 
  ...
  @override void initState() {
    ...
    Connectivity().onConnectivityChanged.listen((result) { 
      ...
      if (_isError && hasNet && !_isCoolingDown && !_isRetrying) {
        ...
        if (mounted) {
          setState(() { _isError = true; _lastKnownNetworkPosition = player.state.position; }); 
        }
      }
    });
    ...
    player.stream.playing.listen((p) { 
      if (p && _thumb && mounted) {
        setState(() => _thumb = false);
      }
      _isPlayingOptimistic.value = p;
      if (p) { 
    ...
    player.stream.volume.listen((v) { 
      _isMutedOptimistic.value = v == 0; 
    });
  }

  void _detectSafeThreading() {
    if (kIsWeb) {
      try {
        final bool isIsolated = web.window.crossOriginIsolated;
        if (isIsolated) {
          debugPrint("✅ SAFE-THREADING: COOP/COEP Detected. WASM Threads Enabled.");
        } else {
          debugPrint("⚠️ FALLBACK: No COOP/COEP. Using Async Future Fallback to prevent Spectre Crash.");
        }
      } catch (_) {}
    }
  }
  ...
  Future<void> _checkSavedPosition() async {
    ...
    if (savedSeconds != null && savedSeconds > 5) {
      if (mounted) {
        setState(() { _savedPosition = Duration(seconds: savedSeconds); _showResumePrompt = true; });
      }
    }
  }
  ...
  void _resetAndPlayNewSource() async { 
    ...
    player.stop(); 
    setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isPlayingOptimistic.value = true; _showResumePrompt = false; }); 
  ...
  void _handleGlobalTap() { 
    if (_showControls && _menuState == SettingsMenuState.none) { 
      ...
      setState(() { _showControls = true; _menuState = SettingsMenuState.none; }); 
      _startControlsTimer(); 
    } 
  }
  ...
  Future<String> _performLinkRace(List<String> urls) async { 
    ...
    Map<String, int> l = {}; 
    await Future.wait(urls.map((url) async { try { final s = Stopwatch()..start(); await http.head(Uri.parse(url)).timeout(const Duration(seconds: 2)); s.stop(); l[url] = s.elapsedMilliseconds; } catch (e) { l[url] = 99999; } })); 
  ...
  void _triggerFailover() {
    ...
    if (_currentLinkIndex + 1 < _currentUrlList.length) { 
      ...
      player.stop(); 
      setState(() { _isError = true; _isCoolingDown = true; }); 
  ...
  void _playUrl(String url, {bool keepPosition = false}) { 
    ...
    _applyNativeSubtitle(); 
  }

  void _applyNativeSubtitle() {
    if (_currentSubtitleLang == "Off") {
      player.setSubtitleTrack(SubtitleTrack.no());
      return;
    }

    String urlToLoad = _vttEn; 
    String langToCheck = _currentSubtitleLang;

    if (langToCheck == "Auto") {
      langToCheck = PlatformDispatcher.instance.locale.languageCode;
      if (langToCheck == 'ar') {
        urlToLoad = _vttAr;
      } else if (langToCheck == 'fr') {
        urlToLoad = _vttFr;
      } else {
        urlToLoad = _vttEn;
      }
    } else {
      if (langToCheck == "Arabic") {
        urlToLoad = _vttAr;
      } else if (langToCheck == "French") {
        urlToLoad = _vttFr;
      } else {
        urlToLoad = _vttEn;
      }
    ...
    try {
      player.setSubtitleTrack(SubtitleTrack.uri(urlToLoad));
    } catch (_) {}
  }

  void _retryPlayback() { 
    if (_isCoolingDown || _isRetrying) {
      return; 
    }
    ...
    Future.delayed(const Duration(seconds: 3), () { 
      if (mounted) {
        setState(() { _isCoolingDown = false; _isRetrying = false; }); 
      }
    });
  }
  ...
  @override void dispose() { 
    ...
    _pulseTimer?.cancel();
    _isPlayingOptimistic.dispose();
    _isMutedOptimistic.dispose();
    player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); super.dispose(); 
  }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    _isBlocked ? Container(color: Colors.black, width: double.infinity, height: double.infinity, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.lock_clock, color: Colors.white54, size: 50), const SizedBox(height: 16), Text(p.tr('protected'), style: const TextStyle(color: Colors.white, fontSize: 18)), Text(p.tr('wait_msg'), style: const TextStyle(color: Colors.white54))])) 
               : Video(
                   ...
                   subtitleViewConfiguration: SubtitleViewConfiguration(
                     ...
                     style: TextStyle(
                       fontSize: (cs.maxWidth * 0.04).clamp(14.0, 36.0),
                       color: Colors.white,
    ...
    if (_thumb) FutureBuilder<Uint8List?>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data != null) {
          return AnimatedOpacity(opacity: _thumb ? 1.0 : 0.0, duration: const Duration(milliseconds: 500), child: Image.memory(snapshot.data!, fit: BoxFit.cover, width: double.infinity, height: double.infinity));
        }
    ...
    if (!_isAd && !_isIntervalAd && _thumb && !_isError) 
      RepaintBoundary(
        child: ValueListenableBuilder<bool>(
          valueListenable: _isPlayingOptimistic,
          builder: (c, playing, _) => (!playing) ? Center(child: IconButton(iconSize: 64, icon: const Icon(Icons.play_circle_fill, color: Colors.white), onPressed: () => player.play())) : const SizedBox.shrink(),
        )
      ),
  ...
  Widget _buildControlsUI(AppProvider p) => Positioned(bottom: 0, left: 0, right: 0, child: Container(color: Colors.black54, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: SafeArea(top: false, child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    Row(children: [
      ValueListenableBuilder<bool>(
        valueListenable: _isPlayingOptimistic,
        builder: (c, playing, _) => IconButton(icon: Icon(playing ? Icons.pause : Icons.play_arrow, color: Colors.white), onPressed: () { _isPlayingOptimistic.value = !playing; player.playOrPause(); })
      ), 
      ValueListenableBuilder<bool>(
        valueListenable: _isMutedOptimistic,
        builder: (c, muted, _) => IconButton(icon: Icon(muted ? Icons.volume_off : Icons.volume_up, color: Colors.white), onPressed: () { _isMutedOptimistic.value = !muted; player.setVolume(player.state.volume == 0 ? 100 : 0); })
      ),
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    ...
    if (_menuState == SettingsMenuState.speed) {
      return [ListTile(dense: true, title: Text(p.tr('back'), style: const TextStyle(color: Colors.amber)), onTap: () => setState(() => _menuState = SettingsMenuState.main)), ...[0.5, 1.0, 1.5, 2.0].map((s) => ListTile(dense: true, title: Text("${s}x", style: TextStyle(color: player.state.rate == s ? Colors.amber : Colors.white)), onTap: () => setState(() { player.setRate(s); })))];
    }
    ...
    if (_menuState == SettingsMenuState.subtitles) {
      return [
        ListTile(dense: true, title: Text(p.tr('back'), style: const TextStyle(color: Colors.amber)), onTap: () => setState(() => _menuState = SettingsMenuState.main)), 
        ListTile(dense: true, title: Text(p.tr('disable'), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == "Off" ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = "Off"; _applyNativeSubtitle(); }); }),
        const Divider(color: Colors.white10),
        ..._availableLangs.map((l) => ListTile(dense: true, title: Text(l == "Auto" ? p.tr('auto') : p.tr(l.toLowerCase()), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == l ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = l; _applyNativeSubtitle(); }); }))
      ];
    }
    return [];
  }
