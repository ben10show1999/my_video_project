...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  String _currentSubtitleKey = "Auto"; // Task 22 Default
  ...
  void _handleSubtitles() { if (_currentSubtitleKey != "Off") { VttSubtitleEngine.loadSubtitles(_currentSubtitleKey, widget.subtitles); } else { VttSubtitleEngine.stop(); } }
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    ...
    // Task 24: Dynamic Menu Generation
    if (_menuState == SettingsMenuState.subtitles) { return [ListTile(dense: true, title: Text(p.tr('back'), style: const TextStyle(color: Colors.amber)), onTap: () => setState(() => _menuState = SettingsMenuState.main)), ListTile(dense: true, title: Text(p.tr('disable'), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleKey == "Off" ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleKey = "Off"; _handleSubtitles(); }); }), ListTile(dense: true, title: Text(p.tr('auto'), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleKey == "Auto" ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleKey = "Auto"; _handleSubtitles(); }); }), ...widget.subtitles.keys.map((langKey) => ListTile(dense: true, title: Text(langKey.toUpperCase(), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleKey == langKey ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleKey = langKey; _handleSubtitles(); }); }))]; }
    return [];
  }
}
