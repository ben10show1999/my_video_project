...
class SmartMediaKitPlayer extends StatefulWidget {
  ...
  // Added parameter for Route-based Fullscreen state management
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  bool _isRetrying = false; // Lock for Retry Storm
...
""";
  ...
  @override void initState() {
    ...
    // Connectivity Listener for Anti-Freeze
    Connectivity().onConnectivityChanged.listen((result) { 
      bool hasNet = !result.contains(ConnectivityResult.none);
      if (_isError && hasNet && !_isCoolingDown) {
         _retryPlayback(); 
      } else if (!hasNet) {
         if (player.state.playing) player.stop(); // CRITICAL: Kill engine immediately
  ...
  void didUpdateWidget(SmartMediaKitPlayer oldWidget) {
    ...
    if (widget.sources != oldWidget.sources && !widget.isFullscreenRoute) {
  ...
  // ROBUST RETRY LOGIC (Hard Reload + Boolean Lock)
  void _retryPlayback() { 
    ...
    Future.delayed(const Duration(seconds: 3), () {
      if(mounted) setState(() { _isCoolingDown = false; _isRetrying = false; });
    });
  }
  
  // BREAKOUT ROUTE FULLSCREEN (Context-Aware Landscape/Portrait)
  void _executeSmartFullscreen() async { 
    if (widget.isFullscreenRoute) {
    ...
    // Pause briefly to sync state before pushing route
    Duration currentPos = player.state.position;
    bool wasPlaying = player.state.playing;
    
    // ✅ The Fix for use_build_context_synchronously
    if (!mounted) return; 

    await Navigator.push(context, MaterialPageRoute(builder: (_) => Scaffold(
      backgroundColor: Colors.black,
      body: SmartMediaKitPlayer(
        sources: widget.sources,
        onAspectRatioChanged: widget.onAspectRatioChanged,
        isFullscreenRoute: true, // Tell the new route it is the fullscreen instance
      ),
    )));
  ...
  @override void dispose() { 
    // Only dispose player if it's the root instance
    if (!widget.isFullscreenRoute) player.dispose(); 
    _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel();
    super.dispose(); 
  }

  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return AspectRatio(aspectRatio: widget.isFullscreenRoute && _currentRatio < 1 ? MediaQuery.of(context).size.aspectRatio : _currentRatio, child: LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    _isBlocked ? Container(color: Colors.black, width: double.infinity, height: double.infinity, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.lock_clock, color: Colors.white54, size: 50), const SizedBox(height: 16), Text(p.tr('protected'), style: const TextStyle(color: Colors.white, fontSize: 18)), Text(p.tr('wait_msg'), style: const TextStyle(color: Colors.white54))])) 
               : Video(controller: controller, controls: NoVideoControls, fit: (widget.isFullscreenRoute && _currentRatio < 1) ? BoxFit.cover : BoxFit.contain), 
    
    if (widget.thumbnailUrl != null && !widget.isFullscreenRoute) AnimatedOpacity(opacity: _thumb ? 1.0 : 0.0, duration: const Duration(milliseconds: 500), child: IgnorePointer(ignoring: !_thumb, child: UniversalImage(path: widget.thumbnailUrl!, fit: BoxFit.cover, width: double.infinity, height: double.infinity))), 
    
    // 2. ABSOLUTE TAP-TO-HIDE SENSOR (Opaque Layer)
    Positioned.fill(child: GestureDetector(
      behavior: HitTestBehavior.opaque, // CRITICAL: Captures all taps before Video
      onTap: _handleGlobalTap,
      child: Container(color: Colors.transparent),
    )),

    // 3. CENTER PLAY BUTTON
    if (!_isAd && !_isIntervalAd && _thumb && !player.state.playing && !_isError && !widget.isFullscreenRoute) Center(child: IconButton(iconSize: 64, icon: const Icon(Icons.play_circle_fill, color: Colors.white), onPressed: () => player.play())), 
  
    // 4. ERROR & COOL-DOWN UI
    if (_isError || _isCoolingDown) Positioned.fill(child: Container(color: Colors.black87, child: Center(child: FittedBox(fit: BoxFit.scaleDown, child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    if (!_isAd && !_isIntervalAd && !_isBlocked && !_isError && _showControls) 
       // Stop controls from passing tap to the hide-layer
       GestureDetector(
    ...
    Positioned(
      top: 16, left: 16,
      child: SafeArea(
        child: Image.asset('assets/images/Club2.webp', width: cs.maxWidth < 600 ? 40 : 65, height: cs.maxWidth < 600 ? 40 : 65, errorBuilder: (c,e,s) => const SizedBox.shrink()),
      ),
    ),
  ])); })); }
  
  // CONTROLS WITH DECOUPLED SLIDER (Optimistic Seek)
  Widget _controls() => Positioned(bottom: 0, left: 0, right: 0, child: Container(color: Colors.black54, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: SafeArea(top: false, child: Column(mainAxisSize: MainAxisSize.min, children: [
    SliderTheme(data: SliderThemeData(trackHeight: 2, thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 6)), child: StreamBuilder<Duration>(stream: player.stream.position, builder: (c, s) { 
        ...
        return Row(children: [Text(_fmt(Duration(seconds: currentVal.toInt())), style: const TextStyle(color: Colors.white, fontSize: 12)), Expanded(child: Slider(
          value: currentVal.clamp(0, dur.inSeconds.toDouble()), 
          min: 0, max: dur.inSeconds.toDouble() > 0 ? dur.inSeconds.toDouble() : 1, 
          activeColor: Colors.amber, inactiveColor: Colors.white24, 
          onChangeStart: (v) => setState(() => _draggingValue = v),
          onChanged: (v) => setState(() => _draggingValue = v), // Visual Update
          onChangeEnd: (v) { 
             player.seek(Duration(seconds: v.toInt())); // Actual Seek
    ...
    Row(children: [
      ...
      const Spacer(), 
      IconButton(icon: Icon(widget.isFullscreenRoute ? Icons.fullscreen_exit : Icons.fullscreen, color: Colors.white), onPressed: _executeSmartFullscreen), 
