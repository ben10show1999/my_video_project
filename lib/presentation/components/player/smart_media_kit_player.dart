...
class SmartMediaKitPlayer extends StatefulWidget {
  ...
  final String? subtitleUrlEn; final String? subtitleUrlAr; final String? adScript;
  const SmartMediaKitPlayer({super.key, required this.sources, required this.videoTitle, required this.videoId, this.totalDuration, this.thumbnailUrl, required this.onAspectRatioChanged, this.isFullscreen = false, required this.onFullscreenToggle, this.subtitleUrlEn, this.subtitleUrlAr, this.adScript});
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  bool _isCoolingDown = false; bool _isRetrying = false; bool _isSwitchingSource = false; 
  String _currentSubtitleLang = "Auto";
  final List<String> _availableLangs = ["Auto", "English", "Arabic"];
  ...
  // Task 12: HLS Support Variables
  bool _isHlsMode = false;
  String _hlsViewId = "";
  Key _hlsKey = UniqueKey();
  ...
  @override void initState() {
    ...
    player.stream.videoParams.listen((params) { 
      ...
      if (w != null && h != null && h > 0) { 
        double nr = w / h; 
        if ((nr - _currentRatio).abs() > 0.01) { setState(() => _currentRatio = nr); widget.onAspectRatioChanged(nr); } 
      } 
    });
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await _checkSavedPosition(); _initializeSmartStream(); _injectNativePreloader(); _startAutoSaveTimer(); _startSmartPulseMonitor();
    });
    
    Connectivity().onConnectivityChanged.listen((result) { 
      bool hasNet = !result.contains(ConnectivityResult.none);
      if (_isError && hasNet && !_isCoolingDown && !_isRetrying) { _retryPlayback(); }
      else if (!hasNet) { 
    ...
    if (widget.adScript != null && widget.adScript!.isNotEmpty) {
      _intervalAdTimer = Timer.periodic(const Duration(minutes: 30), (timer) { if (player.state.playing && !_isIntervalAd && !_isAd && !_isBlocked && !_isHlsMode) { _triggerIntervalAd(); } });
    }
  ...
  void _startAutoSaveTimer() {
    ...
    _savePositionTimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      if (player.state.playing && player.state.position.inSeconds > 5 && !_isHlsMode) {
        final prefs = await SharedPreferences.getInstance();
  ...
  Future<void> _resumeFromPrompt() async {
    if (_savedPosition.inSeconds > 0 && !_isHlsMode) {
  ...
  // ✅ Task 15: Player Lifecycle Fix (ID Watcher)
  // Ensures player updates correctly when navigating between episodes
  @override 
  void didUpdateWidget(SmartMediaKitPlayer oldWidget) { 
  ...
  void _resetAndPlayNewSource() async { 
    ...
    player.stop(); 
    setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isPlayingOptimistic = true; _showResumePrompt = false; _isHlsMode = false; }); 
    await _initializeSmartStream(); 
  }

  void _startControlsTimer() { _controlsTimer?.cancel(); if (mounted) { setState(() => _showControls = true); } _controlsTimer = Timer(const Duration(seconds: 3), () { if (mounted && player.state.playing && _menuState == SettingsMenuState.none && !_isHlsMode) { setState(() => _showControls = false); } }); }
  ...
  Future<void> _autoSelectBestQualityAndLink() async { var q = widget.sources.firstWhere((e) => e.quality == "1080p" || e.quality == "Auto", orElse: () => widget.sources.first); _currentUrlList = q.urls; _playUrl(await _performLinkRace(q.urls)); }
  ...
  // ✅ Task 12: Universal Format Engine logic
  void _playUrl(String url, {bool keepPosition = false}) { 
    ...
    setState(() { _isError = false; _isHlsMode = false; }); 
    ...
    if (keepPosition || _lastKnownNetworkPosition.inSeconds > 0) { Future.delayed(const Duration(milliseconds: 500), () { player.seek(c); _lastKnownNetworkPosition = Duration.zero; }); }
    _handleSubtitles(); 
  }

  void _initHlsEngine(String url) {
    setState(() { _isHlsMode = true; _thumb = false; _hlsViewId = 'hls_engine_${DateTime.now().millisecondsSinceEpoch}'; _hlsKey = UniqueKey(); });
    
    final htmlContent = """
    <!DOCTYPE html><html><head><style>body{margin:0;background:#000;}video{width:100vw;height:100vh;object-fit:contain;}</style></head>
    ...
    <script>
      ...
      if (Hls.isSupported()) {
        ...
        video.src = '$url'; video.addEventListener('loadedmetadata', function() { video.play(); });
      }
    </script></body></html>
    """;

    ui_web.platformViewRegistry.registerViewFactory(_hlsViewId, (int viewId) {
  ...
  void _handleSubtitles() { if (_currentSubtitleLang != "Off" && !_isHlsMode) { VttSubtitleEngine.loadSubtitles(_currentSubtitleLang, enUrl: widget.subtitleUrlEn, arUrl: widget.subtitleUrlAr); } else { VttSubtitleEngine.stop(); } }

  void _retryPlayback() { 
    if (_isCoolingDown || _isRetrying) { return; } 
    setState(() { _isError = false; _isCoolingDown = true; _isRetrying = true; });
    
    if (_isHlsMode) { _initHlsEngine(_activeUrl); } 
    else {
      player.open(Media(_activeUrl)); 
      Future.delayed(const Duration(milliseconds: 500), () { if (_lastKnownNetworkPosition.inSeconds > 0) { player.seek(_lastKnownNetworkPosition); _lastKnownNetworkPosition = Duration.zero; } });
    }
  ...
  void _toggleFullscreenLogic() {
    try {
      if (!widget.isFullscreen) {
        ...
        if (_currentRatio > 1.0) { web.window.screen.orientation.lock('landscape'); } else { web.window.screen.orientation.lock('portrait'); } 
      } else {
        web.document.exitFullscreen(); web.window.screen.orientation.unlock();
      }
    } catch (e) { /* Ignore */ }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    _isBlocked ? Container(color: Colors.black, width: double.infinity, height: double.infinity, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.lock_clock, color: Colors.white54, size: 50), const SizedBox(height: 16), Text(p.tr('protected'), style: const TextStyle(color: Colors.white, fontSize: 18)), Text(p.tr('wait_msg'), style: const TextStyle(color: Colors.white54))])) 
               : (_isHlsMode ? HtmlElementView(key: _hlsKey, viewType: _hlsViewId) : Video(controller: controller, controls: NoVideoControls, fit: BoxFit.contain)), 
    
    if (!_isHlsMode) Positioned.fill(child: AspectRatio(aspectRatio: _currentRatio, child: Align(alignment: Alignment.bottomCenter, child: Padding(padding: EdgeInsets.only(bottom: cs.maxHeight * 0.12), child: ValueListenableBuilder<String>(valueListenable: VttSubtitleEngine.currentSubtitle, builder: (context, text, child) { if (text.isEmpty || _currentSubtitleLang == "Off") { return const SizedBox.shrink(); } double fontSize = (cs.maxWidth * 0.04).clamp(14.0, 36.0); return Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: Colors.black87, borderRadius: BorderRadius.circular(8)), child: Text(text, textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: fontSize, fontWeight: FontWeight.bold, shadows: const [Shadow(color: Colors.black, offset: Offset(1, 1), blurRadius: 2)]))); }))))),
    if (_thumb && !_isHlsMode) FutureBuilder<Uint8List?>(future: ThumbnailCache.getThumbnail(widget.thumbnailUrl ?? (widget.sources.isNotEmpty ? widget.sources.first.urls.first : "")), builder: (context, snapshot) { if (snapshot.connectionState == ConnectionState.waiting) { return Container(color: Colors.black, child: const Center(child: CircularProgressIndicator(color: Colors.amber))); } if (snapshot.hasData && snapshot.data != null) { return AnimatedOpacity(opacity: _thumb ? 1.0 : 0.0, duration: const Duration(milliseconds: 500), child: Image.memory(snapshot.data!, fit: BoxFit.cover, width: double.infinity, height: double.infinity)); } return Container(color: Colors.black); }),
    if (!_isHlsMode) Positioned.fill(child: GestureDetector(behavior: HitTestBehavior.translucent, onTap: _handleGlobalTap, child: Container(color: Colors.transparent))),
    if (!_isAd && !_isIntervalAd && _thumb && !player.state.playing && !_isError && !_isHlsMode) Center(child: IconButton(iconSize: 64, icon: const Icon(Icons.play_circle_fill, color: Colors.white), onPressed: () => player.play())), 
    ...
    if (player.state.buffering && !_isActuallyAdvancing && !_isError && !_isHlsMode) const Center(child: CircularProgressIndicator(color: Colors.amber)),
    if (!_isAd && !_isIntervalAd && !_isBlocked && !_isError && _showControls && !_isHlsMode) _buildControlsUI(p), 
    if (!_isAd && !_isIntervalAd && _menuState != SettingsMenuState.none && !_isHlsMode) _buildMenuUI(cs.biggest, p), 
    if ((_isAd || _isIntervalAd) && widget.adScript != null) Positioned.fill(child: WebAdBanner(adScript: widget.adScript, onClose: _closeAd)),
    // Task 14: Dynamic Player Logo
    Positioned(top: 16, left: 16, child: SafeArea(child: SizedBox(width: (cs.maxWidth * 0.06).clamp(30.0, 70.0), child: UniversalImage(path: RemoteConfigService.branding.playerLogoUrl, fit: BoxFit.contain)))),
    if (_showResumePrompt && !_isHlsMode) Positioned(bottom: 24, left: 16, right: 16, child: SafeArea(child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), decoration: BoxDecoration(color: const Color(0xFF323232), borderRadius: BorderRadius.circular(8), boxShadow: const [BoxShadow(color: Colors.black45, blurRadius: 10, offset: Offset(0, 4))]), child: Directionality(textDirection: TextDirection.rtl, child: Row(children: [Expanded(child: Text(p.tr('resume_msg'), style: const TextStyle(color: Colors.white, fontSize: 14))), TextButton(onPressed: _resumeFromPrompt, child: Text(p.tr('resume_btn'), style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold))), IconButton(icon: const Icon(Icons.close, color: Colors.white70, size: 20), padding: EdgeInsets.zero, constraints: const BoxConstraints(), onPressed: () => setState(() => _showResumePrompt = false))])))))
  ])); }); }
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    if (_menuState == SettingsMenuState.main) { return [ListTile(dense: true, title: Text(p.tr('speed'), style: const TextStyle(color: Colors.white)), onTap: () => setState(() => _menuState = SettingsMenuState.speed), trailing: const Icon(Icons.chevron_right, color: Colors.white)), const Divider(color: Colors.white10), ListTile(dense: true, title: Text(p.tr('quality'), style: const TextStyle(color: Colors.white)), subtitle: Text(_currentQualityLabel, style: const TextStyle(color: Colors.amber, fontSize: 10)), onTap: () => setState(() => _menuState = SettingsMenuState.quality), trailing: const Icon(Icons.chevron_right, color: Colors.white)), const Divider(color: Colors.white10), ListTile(dense: true, title: Text(p.tr('subtitles'), style: const TextStyle(color: Colors.white)), subtitle: Text(_currentSubtitleLang, style: const TextStyle(color: Colors.amber, fontSize: 10)), onTap: () => setState(() => _menuState = SettingsMenuState.subtitles), trailing: const Icon(Icons.chevron_right, color: Colors.white))]; }
    ...
    if (_menuState == SettingsMenuState.subtitles) { return [ListTile(dense: true, title: Text(p.tr('back'), style: const TextStyle(color: Colors.amber)), onTap: () => setState(() => _menuState = SettingsMenuState.main)), ListTile(dense: true, title: Text(p.tr('disable'), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == "Off" ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = "Off"; _handleSubtitles(); }); }), ..._availableLangs.map((l) => ListTile(dense: true, title: Text(l == "Auto" ? p.tr('auto') : p.tr(l.toLowerCase()), style: const TextStyle(color: Colors.white)), trailing: _currentSubtitleLang == l ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { setState(() { _currentSubtitleLang = l; _handleSubtitles(); }); }))]; }
    return [];
  }
}
