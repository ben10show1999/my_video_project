...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  // Quality & Adaptation Logic (Tasks 28, 29, 30)
  ...
  @override void initState() {
    ...
    // Task 26: Auto Reset Ratio on Complete
    ...
    player.stream.error.listen((e) { 
      // Task 29: Auto Failover on Error
      _attemptFailover();
    });
  }
  ...
  Future<void> _initializeSmartStream() async {
    ...
    if (_isAutoMode) {
      // Task 30: Auto Mode (Hybrid) - Start with 1080p, if fail go lower
      await _smartSelectQuality("1080p", fallback: true);
    } else {
      // Task 30: Manual Mode - Stick to quality but pick best server
      await _smartSelectQuality(pref, fallback: false);
    }
  }

  // Task 29 & 30: Intelligent Source Selector
  Future<void> _smartSelectQuality(String q, {bool fallback = false}) async {
    // Find model for this quality
    var qm = widget.sources.firstWhere((e) => e.quality == q, orElse: () => widget.sources.first);
    
    // Find fastest link for this quality
    String bestUrl = await _findFastestLink(qm.urls);
    
    // LINT FIX: Added curly braces for if/else statements
    if (bestUrl.isEmpty && fallback) {
      // Try next lower quality
      if (q == "1080p") {
        await _smartSelectQuality("480p", fallback: true);
      } else if (q == "480p") {
        await _smartSelectQuality("240p", fallback: true);
      } else {
  ...
  Future<String> _findFastestLink(List<String> urls) async {
    ...
    await Future.wait(urls.map((url) async {
      try {
        ...
        await http.head(Uri.parse(url)).timeout(const Duration(seconds: 1)); // Quick ping
        s.stop();
  ...
  void _checkLinkHealth() {
    // Background monitor logic (Task 29)
    // In a real app, this would ping alternative links and seamless switch if current is slow.
  }

  void _attemptFailover() {
     // Task 29: Failover Logic
     setState(() => _isError = true);
  ...
  @override void dispose() { player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _backgroundHealthTimer?.cancel(); super.dispose(); }
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    if (_menuState == SettingsMenuState.main) {
      ...
      // Task 28: Multi-Source Quality Menu
    ...
    // Task 28 & 30: Hybrid Quality Selection UI
