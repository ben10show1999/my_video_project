...
class SmartMediaKitPlayer extends StatefulWidget {
  ...
  const SmartMediaKitPlayer({super.key, required this.sources, this.thumbnailUrl, required this.onAspectRatioChanged, this.isFullscreen = false, required this.onFullscreenToggle});
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    super.initState();
    AiSubtitleEngine.initWorker(); // Init AI background worker lazy
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _injectNativePreloader(); // ✅ Task 6: Native Preloader
    });
    ...
    player.stream.playing.listen((p) { 
      ...
      if (p) { _isSwitchingSource = false; _startActiveTimer(); _startControlsTimer(); } 
      else {
        _pauseActiveTimer();
      } 
    });
  ...
  // ✅ Task 6: Native Preload Injection
  ...
  void _playUrl(String url, {bool keepPosition = false}) { 
    ...
    _handleAiSubtitles(); // Trigger AI Subtitles
  }

  // ✅ Task 7 & 8: AI Subtitles Processing
  void _handleAiSubtitles() {
    if (_isAiSubtitlesEnabled && _currentSubtitleLang != "Off") {
      String target = _currentSubtitleLang == "Auto" ? "ar" : _currentSubtitleLang.substring(0,2).toLowerCase();
      AiSubtitleEngine.processVideo(_activeUrl, target);
    } else {
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    // ✅ Task 7: Subtitles Responsive Overlay
    Positioned(
        ...
        builder: (context, text, child) {
          ...
          return Center(
            child: Container(
              ...
              child: Text(text, textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: max(14, cs.maxWidth * 0.03), fontWeight: FontWeight.bold, shadows: const [Shadow(color: Colors.black, offset: Offset(1, 1), blurRadius: 2)])),
            ),
          );
        }
      ),
