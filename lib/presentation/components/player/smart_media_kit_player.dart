...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  void _handleStreamFailure() async {
    ...
    if (nextUrl != null) {
      ...
      debugPrint("�7�4 Critical: All links failed.");
    }
  }

  // --- �0�8 SMART NETWORK LOGIC END ---
  ...
  @override void dispose() { player.dispose(); _recordingTimer?.cancel(); _networkMonitorTimer?.cancel(); super.dispose(); }
  ...
  List<Widget> _buildMenuContent() {
    switch (_menuState) {
      ...
      case SettingsMenuState.speed: return [
        InkWell(onTap: () => setState(() => _menuState = SettingsMenuState.main), child: const Padding(padding: EdgeInsets.all(8.0), child: Row(children: [Icon(Icons.arrow_back, color: Colors.white, size: 16), SizedBox(width: 8), Text("Speed", style: TextStyle(color: Colors.white))]))),
      ...
      case SettingsMenuState.quality: return [
        InkWell(onTap: () => setState(() => _menuState = SettingsMenuState.main), child: const Padding(padding: EdgeInsets.all(8.0), child: Row(children: [Icon(Icons.arrow_back, color: Colors.white, size: 16), SizedBox(width: 8), Text("Quality", style: TextStyle(color: Colors.white))]))),
        ...
        ...widget.sources.map((q) => ListTile(
          dense: true, 
          title: Text(q.quality, style: TextStyle(color: (!_isAutoMode && _currentQualityLabel == q.quality) ? Colors.amber : Colors.white)), 
          onTap: () async { 
            setState(() { _isAutoMode = false; _currentQualityLabel = q.quality; }); 
            await Provider.of<AppProvider>(context, listen: false).setPreferredQuality(q.quality); 
            await _manualSelectQuality(q.quality); 
          }
        ))
      ];
      case SettingsMenuState.subtitles: return [
        InkWell(onTap: () => setState(() => _menuState = SettingsMenuState.main), child: const Padding(padding: EdgeInsets.all(8.0), child: Row(children: [Icon(Icons.arrow_back, color: Colors.white, size: 16), SizedBox(width: 8), Text("Subtitles", style: TextStyle(color: Colors.white))]))),
      ...
      case SettingsMenuState.aiLanguages: return [
         InkWell(onTap: () => setState(() => _menuState = SettingsMenuState.subtitles), child: const Padding(padding: EdgeInsets.all(8.0), child: Row(children: [Icon(Icons.arrow_back, color: Colors.white, size: 16), SizedBox(width: 8), Text("Select Language", style: TextStyle(color: Colors.white))]))),
