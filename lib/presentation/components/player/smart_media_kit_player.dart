...
import 'package:my_video_project/core/utils/telemetry_service.dart';

enum SettingsMenuState { none, main, speed, quality, subtitles, subtitleLangs }

class SmartMediaKitPlayer extends StatefulWidget {
  final String movieId; final String movieTitle; final List<VideoQualityModel> sources; final String? thumbnailUrl;
  final Function(double ratio) onAspectRatioChanged; final bool isFullscreen; final VoidCallback onFullscreenToggle;
  const SmartMediaKitPlayer({super.key, required this.movieId, required this.movieTitle, required this.sources, this.thumbnailUrl, required this.onAspectRatioChanged, this.isFullscreen = false, required this.onFullscreenToggle});
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  bool _thumb = true; bool _loop = false; bool _showControls = true; Timer? _controlsTimer;
  bool _isError = false; bool _isCoolingDown = false; bool _isRetrying = false; bool _isSwitchingSource = false; 
  ...
  bool _isAutoMode = true; double _currentRatio = 16/9; int _currentLinkIndex = 0; List<String> _currentUrlList = [];
  ...
  @override void initState() {
    ...
    player.stream.playing.listen((p) { 
      if (p && _thumb && mounted) setState(() => _thumb = false); if (mounted) setState(() => _isPlayingOptimistic = p);
  ...
  void _injectNativePreloader() {
    try {
      ...
      for (int i = 0; i < videos.length; i++) {
        ...
        v.setAttribute('preload', 'auto'); v.setAttribute('disablePictureInPicture', 'true');
      }
    } catch (_) {}
  }
  ...
  void _resetAndPlayNewSource() async { _isSwitchingSource = true; player.stop(); setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isPlayingOptimistic = true; }); await _initializeSmartStream(); }
  ...
  Future<void> _initializeSmartStream() async { final provider = Provider.of<AppProvider>(context, listen: false); String pref = provider.preferredQuality; setState(() { _isAutoMode = pref == "Auto"; _currentQualityLabel = pref; }); if (_isAutoMode) {
  ...
  // ✅ Task 7: Telemetry Triggers on fail
  void _triggerFailover() {
    if (_isCoolingDown) return; 
    TelemetryService.reportBrokenLink(movieTitle: widget.movieTitle, quality: _currentQualityLabel, brokenUrl: _activeUrl, movieId: widget.movieId);
  ...
  void _playUrl(String url, {bool keepPosition = false}) { if (url.isEmpty || url == _activeUrl) return; _activeUrl = url; Duration c = player.state.position; setState(() => _isError = false); player.open(Media(url, httpHeaders: {'Cache-Control': 'max-age=3600', 'Range': 'bytes=0-'}), play: true); if (keepPosition) Future.delayed(const Duration(milliseconds: 500), () => player.seek(c)); _handleAiSubtitles(); }
  void _handleAiSubtitles() { if (_isAiSubtitlesEnabled && _currentSubtitleLang != "Off") { String target = _currentSubtitleLang == "Auto" ? "ar" : _currentSubtitleLang.substring(0,2).toLowerCase(); AiSubtitleEngine.processVideo(_activeUrl, target); } else { AiSubtitleEngine.stop(); } }
  void _retryPlayback() { if (_isCoolingDown || _isRetrying) return; setState(() { _isError = false; _isCoolingDown = true; _isRetrying = true; }); player.open(Media(_activeUrl)); Future.delayed(const Duration(seconds: 3), () { if(mounted) setState(() { _isCoolingDown = false; _isRetrying = false; }); }); }
  void _toggleFullscreenLogic() { try { if (!widget.isFullscreen) { web.document.documentElement?.requestFullscreen(); if (_currentRatio > 1.0) {
    web.window.screen.orientation.lock('landscape');
  } else {
    web.window.screen.orientation.lock('portrait');
  } } else { web.document.exitFullscreen(); web.window.screen.orientation.unlock(); } } catch (e) { /* Ignore unsupported API errors silently */ } widget.onFullscreenToggle(); }
  @override void dispose() { AiSubtitleEngine.stop(); player.dispose(); _controlsTimer?.cancel(); super.dispose(); }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    // ✅ Responsive AI Subtitles anchor inside video boundary
    Positioned(bottom: 60, left: 16, right: 16, child: ValueListenableBuilder<String>(valueListenable: AiSubtitleEngine.currentSubtitle, builder: (context, text, child) {
      if (text.isEmpty) return const SizedBox.shrink();
      return Center(child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: Colors.black87, borderRadius: BorderRadius.circular(8)), child: Text(text, textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: max(14, cs.maxWidth * 0.04), fontWeight: FontWeight.bold, shadows: const [Shadow(color: Colors.black, offset: Offset(1, 1), blurRadius: 2)]))));
    })),

    if (_thumb) FutureBuilder<Uint8List?>(future: ThumbnailCache.getThumbnail(widget.thumbnailUrl ?? widget.sources.first.urls.first), builder: (context, snapshot) {
      ...
      if (snapshot.hasData && snapshot.data != null) return AnimatedOpacity(opacity: _thumb ? 1.0 : 0.0, duration: const Duration(milliseconds: 500), child: Image.memory(snapshot.data!, fit: BoxFit.cover, width: double.infinity, height: double.infinity)); return Container(color: Colors.black);
    }),
    ...
    if (player.state.buffering && !_isError) const Center(child: CircularProgressIndicator(color: Colors.amber)),
    
    ValueListenableBuilder<bool>(valueListenable: AiSubtitleEngine.isModelLoading, builder: (context, isLoading, child) {
    ...
    if (!_isError && _showControls) _buildControlsUI(p), 
  ...
  List<Widget> _buildMenuContent(AppProvider p) {
    ...
    if (_menuState == SettingsMenuState.subtitles) {
      ...
      ..._availableLangs.map((l) => ListTile(dense: true, title: Text(l == "Auto" ? p.tr('auto') : p.tr(l.toLowerCase()), style: TextStyle(color: Colors.white.withValues(alpha: _isAiSubtitlesEnabled ? 1.0 : 0.4))), trailing: _currentSubtitleLang == l ? const Icon(Icons.check, color: Colors.amber, size: 16) : null, onTap: () { if (_isAiSubtitlesEnabled) { setState(() { _currentSubtitleLang = l; _handleAiSubtitles(); }); } })) // ✅ Fix applied
    ];
    }
    return [];
  }
