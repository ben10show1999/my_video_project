...
class SmartMediaKitPlayer extends StatefulWidget {
  final List<VideoQualityModel> sources; final String videoTitle; final String? thumbnailUrl;
  final Function(double ratio) onAspectRatioChanged; final bool isFullscreen; final VoidCallback onFullscreenToggle;
  const SmartMediaKitPlayer({super.key, required this.sources, required this.videoTitle, this.thumbnailUrl, required this.onAspectRatioChanged, this.isFullscreen = false, required this.onFullscreenToggle});
...
class _SmartMediaKitPlayerState extends State<SmartMediaKitPlayer> {
  ...
  @override void initState() {
    ...
    Connectivity().onConnectivityChanged.listen((result) { 
      ...
      if (_isError && hasNet && !_isCoolingDown && !_isRetrying) {
        _retryPlayback();
      } else if (!hasNet) { _lastKnownPosition = player.state.position; if (player.state.playing) player.stop(); if (mounted) setState(() => _isError = true); }
    });
  ...
  void _injectNativePreloader() {
    try {
      ...
      for (int i = 0; i < videos.length; i++) {
        ...
        v.setAttribute('preload', 'auto'); v.setAttribute('disableRemotePlayback', 'true');
      }
    } catch (_) { 
      // ✅ RADICAL FIX: Documented Ignore
      /* Ignore DOM query errors silently */ 
    }
  }
  ...
  void _resetAndPlayNewSource() async { _isSwitchingSource = true; _lastKnownPosition = Duration.zero; player.stop(); setState(() { _isError = false; _thumb = true; _isCoolingDown = false; _isPlayingOptimistic = true; }); await _initializeSmartStream(); }
  ...
  void _playUrl(String realUrl, {bool keepPosition = false}) { 
    if (realUrl.isEmpty || realUrl == _activeUrl) return; 
    _activeUrl = realUrl; 
    Duration c = keepPosition ? player.state.position : _lastKnownPosition; 
    setState(() => _isError = false); 
    
    final String proxyUrl = "crypto-stream/?target=${Uri.encodeComponent(realUrl)}";
    player.open(Media(proxyUrl, httpHeaders: {'Cache-Control': 'max-age=3600', 'Range': 'bytes=0-'}), play: true); 
    
    if (c > Duration.zero) Future.delayed(const Duration(milliseconds: 600), () => player.seek(c)); 
    _handleAiSubtitles(); 
  }

  void _handleAiSubtitles() { if (_isAiSubtitlesEnabled && _currentSubtitleLang != "Off") { AiSubtitleEngine.processVideo(_activeUrl, _currentSubtitleLang); } else { AiSubtitleEngine.stop(); } }

  void _retryPlayback() { 
    if (_isCoolingDown || _isRetrying) return; 
    setState(() { _isError = false; _isCoolingDown = true; _isRetrying = true; });
    final String proxyUrl = "crypto-stream/?target=${Uri.encodeComponent(_activeUrl)}";
    player.open(Media(proxyUrl), play: true); 
    if (_lastKnownPosition > Duration.zero) Future.delayed(const Duration(milliseconds: 600), () => player.seek(_lastKnownPosition));
  ...
  void _toggleFullscreenLogic() {
    try {
      if (!widget.isFullscreen) { web.document.documentElement?.requestFullscreen(); if (_currentRatio > 1.0) {
      ...
      // ✅ RADICAL FIX 1: Explicitly documented intentional swallow
      /* Ignore unsupported API errors silently */ 
    }
  ...
  @override void dispose() { web.window.removeEventListener('recording_detected', _recordingDetectedListener); web.window.removeEventListener('recording_stopped', _recordingStoppedListener); AiSubtitleEngine.stop(); player.dispose(); _activeTimer?.cancel(); _cooldownTimer?.cancel(); _intervalAdTimer?.cancel(); _controlsTimer?.cancel(); super.dispose(); }
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return LayoutBuilder(builder: (c, cs) { return MouseRegion(onHover: (_) => _startControlsTimer(), child: Stack(alignment: Alignment.center, children: [
    ...
    if (player.state.buffering && !_isError && !player.state.playing) const Center(child: CircularProgressIndicator(color: Colors.amber)),
    
    ValueListenableBuilder<bool>(valueListenable: AiSubtitleEngine.isModelLoading, builder: (context, isLoading, child) { if (!isLoading || !_isAiSubtitlesEnabled) return const SizedBox.shrink(); return Positioned(top: 16, right: 16, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(4)), child: Row(children: [const SizedBox(width: 12, height: 12, child: CircularProgressIndicator(color: Colors.amber, strokeWidth: 2)), const SizedBox(width: 8), Text(p.tr('ai_loading'), style: const TextStyle(color: Colors.amber, fontSize: 10))]))); }),
  ...
  Widget _buildControlsUI(AppProvider p) => Positioned(bottom: 0, left: 0, right: 0, child: Container(color: Colors.black54, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: SafeArea(top: false, child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    Row(children: [
      ...
      const Spacer(), IconButton(icon: Icon(widget.isFullscreen ? Icons.fullscreen_exit : Icons.fullscreen, color: Colors.white), onPressed: _toggleFullscreenLogic), const SizedBox(width: 8), 
