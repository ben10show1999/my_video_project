...
// Task 26 & 27: Stubborn Container & Geometric Containment
class StubbornAdContainer extends StatefulWidget {
  final VoidCallback onClose;
  final String? scriptCode; // For JS Ads
  final Widget? simpleContent; // For Image Ads
...
class _StubbornAdContainerState extends State<StubbornAdContainer> {
  ...
  Widget build(BuildContext context) {
    ...
    // Task 27: Strict Clipping
    return ClipRect(
      child: Container(
        color: Colors.black.withValues(alpha: 0.8), // Dimmed background
        child: Stack(
            ...
            if (!_isConnected)
              Column(mainAxisSize: MainAxisSize.min, children: [
               ...
               widget.scriptCode != null 
                 ? WebIframeAd(scriptContent: widget.scriptCode!, width: 300, height: 250) // Task 28
                 : AspectRatio(aspectRatio: widget.adAspectRatio, child: widget.simpleContent ?? const SizedBox()),

            // Task 26: Close Button Authority (Only exit)
            Positioned(
              top: 10, right: 10,
