...
import 'package:pointer_interceptor/pointer_interceptor.dart'; // CRITICAL: Fixes click-through
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            top: 40, left: 20,
            child: PointerInterceptor( // Shield
              child: Container(
...
class _WebAdBannerState extends State<WebAdBanner> {
  ...
  void _registerFactory() {
    // ELASTIC GEOMETRY & SCROLL FIX: 
    // 1. width/height 100% (Elastic)
    // 2. scrolling="no" (Fixes mobile scroll)
    // 3. touch-action: pan-y (Fixes gestures)
    final content = """
      <html>
      <head>
        <style>
  ...
  Widget build(BuildContext context) {
    ...
    // ELASTIC WRAPPER: Expand to fill parent
    return SizedBox.expand(
      child: Stack(
        children: [
          ...
          // GLASS SHIELD: Transparent layer to catch taps & allow scroll
          Positioned.fill(
            child: GestureDetector(
               behavior: HitTestBehavior.translucent, // Allows drag to bubble up to List, catches tap
               onTap: () {
                 // Open Floating Page on Tap
          ...
          // CLICK SHIELD: PointerInterceptor to catch event before Iframe
          ...
          if (!widget.isInternal)
            Positioned(
              ...
              child: Center(
                child: PointerInterceptor( // The Magic Fix
                  child: MouseRegion(
