...
class WebAdBanner extends StatefulWidget {
  ...
  const WebAdBanner({super.key, this.adScript, this.isInternal = false, this.onClose});
...
class _WebAdBannerState extends State<WebAdBanner> {
  ...
  @override void initState() {
    super.initState();
    if (widget.adScript == null || widget.adScript!.isEmpty) return;
    ...
    _msgListener = (web.Event event) {
      try {
        ...
        if (data['type'] == 'ad_resize') {
          ...
          if (h > 0 && h != _dynamicHeight && mounted) { setState(() => _dynamicHeight = h); }
        }
      } catch (e) { /* Ignore */ }
    }.toJS;
    ...
    _sub = Connectivity().onConnectivityChanged.listen((res) {
      ...
      if (connected && !_isConnected && !_isReloading) {
        ...
        _cooldownTimer = Timer(const Duration(seconds: 5), () { if (mounted) { setState(() { _isConnected = true; _isReloading = false; _key = UniqueKey(); }); } });
      } else if (!connected) { 
  ...
  // Task 34: Fluid Iframe Injection via CSS for responsive downscaling
  ...
  @override void dispose() { 
    ...
    _sub?.cancel(); _cooldownTimer?.cancel(); super.dispose(); 
  }

  @override Widget build(BuildContext context) {
    if (widget.adScript == null || widget.adScript!.isEmpty) return const SizedBox.shrink();
    final p = Provider.of<AppProvider>(context);
    
    // Task 34: Zero-Clipping Ad Wrap (FittedBox logic to protect Ads on shrink)
    Widget adContent = _isConnected && !_isReloading 
        ? HtmlElementView(key: _key, viewType: _viewId)
        : Container(color: Colors.black, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const CircularProgressIndicator(color: Colors.amber), if (!_isConnected) Text(p.tr('connection_error'), style: const TextStyle(color: Colors.white, fontSize: 10))]));

    Widget fittedAdContent = FittedBox(
      fit: BoxFit.scaleDown,
      alignment: Alignment.topCenter,
      child: SizedBox(
        width: 320, // Standard Ad Width ensuring iframe layout doesn't break
        height: widget.isInternal ? MediaQuery.of(context).size.height : _dynamicHeight,
        child: adContent,
      ),
    );
