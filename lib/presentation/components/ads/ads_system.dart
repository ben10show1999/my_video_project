...
// �7�3 TASK 8 & 9: Reliable Ad Loader with Auto-Retry & Containment
...
class ReliableAdLoaderState extends State<ReliableAdLoader> { 
  ...
  @override void initState() { 
    ...
    _sub = Connectivity().onConnectivityChanged.listen((l) { 
      ...
      if (_err && !l.contains(ConnectivityResult.none)) _retry(); 
    }); 
  } 
  void _retry() { if (mounted) setState(() { _err = false; _k = UniqueKey(); }); } 
  @override void dispose() { _sub?.cancel(); super.dispose(); } 
  @override Widget build(BuildContext context) { 
    if (_err) return Container(color: Colors.black, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.wifi_off, color: Colors.white54), const SizedBox(height: 8), const Text("Waiting for connection...", style: TextStyle(color: Colors.white54, fontSize: 10))])); 
    // Containment Wrapper
    return ClipRect(
      child: Builder(key: _k, builder: (c) => widget.adBuilder(c, widget.adUrl)),
    );
  } 
}
...
class SmartAdContainer extends StatelessWidget { final Widget adContent; final VoidCallback onClose; final String? externalUrl; final double adAspectRatio; const SmartAdContainer({super.key, required this.adContent, required this.onClose, this.externalUrl, this.adAspectRatio = 16/9}); @override Widget build(BuildContext context) { 
  // �7�3 TASK 9: Containment (ConstrainedBox + AspectRatio + BoxFit)
  return AspectRatio(
    ...
    child: Container(
      ...
      child: ClipRRect(
        ...
        child: Stack(children: [
          Positioned.fill(
            child: FittedBox(
              fit: BoxFit.contain, // Prevent stretching/cropping
              child: SizedBox(
                width: 300, height: 168, // Nominal Size for fitting logic
                child: adContent
              )
            )
          ), 
