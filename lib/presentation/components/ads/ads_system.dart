...
class FullscreenAdViewer extends StatelessWidget {
  ...
  Future<void> _openBrowser() async {
    const url = "https://google.com"; // In real app, extract URL from script if possible
    if (await canLaunchUrl(Uri.parse(url))) {
      // FIX: Ensure externalApplication mode for WebView compatibility
...
// --- Main Banner Widget (Anti-Freeze + Click Shield) ---
...
class _WebAdBannerState extends State<WebAdBanner> {
  ...
  bool _isReloading = false; // Anti-Freeze
  ...
  @override
  Widget build(BuildContext context) {
    // FIX: Tight Fit Logic - Using SizedBox to enforce EXACT dimensions
    return Center(
      child: FittedBox(
        fit: BoxFit.scaleDown, // Only shrink if screen is smaller
        alignment: Alignment.center,
        child: SizedBox(
          width: widget.width,   // Strict Width (300)
          height: widget.height, // Strict Height (250)
          child: Stack(
            children: [
              // 1. The Ad Iframe
              ...
              // 2. Close Button (Task: Center Top, Transparent, High HitTest)
              if (!widget.isInternal)
                Positioned(
                  top: 0, left: 0, right: 0,
                  child: Center(
                    child: GestureDetector(
                      // FIX: Force opaque hit test to catch clicks over Iframe
                      behavior: HitTestBehavior.opaque,
                      onTap: widget.onClose,
                      child: Container(
                        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
                        decoration: BoxDecoration(
                          color: Colors.black.withValues(alpha: 0.7), // Slightly darker for visibility
                        ...
                        child: const Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Text("Close AD ", style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
              ...
              // 3. Expand Button (Task: Bottom Left, High HitTest)
              if (!widget.isInternal)
                Positioned(
                  bottom: 4, left: 4,
                  child: GestureDetector(
