...
class _WebAdBannerState extends State<WebAdBanner> {
  ...
  @override void initState() { 
    ...
    _msgListener = (web.Event event) { 
      ...
      } catch (e) { debugPrint('Ad resize ignored: $e'); } 
    }.toJS; 
  ...
  void _registerFluidFactory() { 
    final content = """
      ...
      <style> 
        html, body { margin: 0; padding: 0; width: 100vw; height: 100vh; overflow: hidden; display: flex; align-items: center; justify-content: center; background-color: transparent; } 
        iframe, div, img { max-width: 100vw; max-height: 100vh; transform-origin: top center; object-fit: contain; } 
      </style> </head> 
        ...
        <script> 
          ...
          const ro = new ResizeObserver(entries => { 
            for (let entry of entries) { 
              window.parent.postMessage(JSON.stringify({ type: 'ad_resize', height: entry.contentRect.height }), '*'); 
            } 
          }); 
          ro.observe(wrapper); 
        </script> 
      ...
      : Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (!_isConnected) Text(p.tr('connection_error'), style: const TextStyle(color: Colors.white, fontSize: 10))
            ]
          )
        );
                ...
                child: PointerInterceptor(
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: Colors.black.withValues(alpha: 0.8), borderRadius: const BorderRadius.only(bottomLeft: Radius.circular(12), bottomRight: Radius.circular(12)), border: Border.all(color: Colors.white24)), 
