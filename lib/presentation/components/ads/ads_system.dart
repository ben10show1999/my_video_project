...
class _WebAdBannerState extends State<WebAdBanner> {
  ...
  bool _isReloading = false; // Anti-Freeze for Ads
  ...
  void initState() {
    ...
    _sub = Connectivity().onConnectivityChanged.listen((res) {
      ...
      if (connected && !_isConnected && !_isReloading) {
        // Anti-Freeze: Wait 3 seconds before reloading
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: FittedBox(
        ...
        alignment: Alignment.center,
        child: SizedBox(
          width: widget.width,   // Exact width
          height: widget.height, // Exact height
          child: Stack(
            children: [
              // 1. The Ad Iframe
              ...
              // 2. Close Button (Restored: Top Center & Transparent)
              if (!widget.isInternal)
                Positioned(
                  top: 0, left: 0, right: 0, // Centered horizontally
                  child: Center(
                    child: InkWell(
