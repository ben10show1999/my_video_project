...
class FullscreenAdViewer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          // TASK 2: Force Fullscreen Elasticity
          Positioned.fill(
             child: WebAdBanner(
              adScript: adScript,
...
class _WebAdBannerState extends State<WebAdBanner> {
  ...
  void initState() {
    ...
    _sub = Connectivity().onConnectivityChanged.listen((res) {
      ...
      if (connected && !_isConnected && !_isReloading) {
        // TASK 1 IMPLEMENTATION: Wait 5 seconds before retrying to prevent UI freeze
        _cooldownTimer?.cancel();
        setState(() => _isReloading = true); // Show loading immediately
  ...
  void _registerFactory() {
    // TASK 2 (Elasticity): CSS injection to force 100% width/height and cover
    final content = """
      <html>
      <head><style>
  ...
  @override
  Widget build(BuildContext context) {
    // TASK 2 (Elasticity): SizedBox.expand forces the HTML view to fill parent
    return SizedBox.expand(
      child: Stack(
        children: [
          ...
          // TASK 2 (Click Shield): High-Priority Close Button
              ...
              child: Center(
                child: MouseRegion(
                  ...
                  child: GestureDetector(
                    // CRITICAL: Opaque behavior intercepts clicks before Iframe gets them
                    behavior: HitTestBehavior.opaque, 
                    onTap: () {
                      debugPrint("Shield: Close Clicked"); 
                      widget.onClose();
                    },
                    child: Container(
          ...
          // TASK 2 (Click Shield): High-Priority Expand Button
          if (!widget.isInternal)
            Positioned(
              bottom: 8, left: 8,
              child: MouseRegion(
                cursor: SystemMouseCursors.click,
                child: GestureDetector(
                  behavior: HitTestBehavior.opaque, // CRITICAL
                  onTap: () {
