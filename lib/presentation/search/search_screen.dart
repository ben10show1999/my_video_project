...
import 'package:my_video_project/presentation/details/movie_details_screen.dart' deferred as details_screen;
...
class _S extends State<SearchScreen> { 
  ...
  @override Widget build(BuildContext context) { 
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text(p.tr('search')), centerTitle: true, backgroundColor: Colors.transparent, elevation: 0, leading: BackButton(color: Colors.white, onPressed: () { if (p.navIndex != 0) {
        p.setNavIndex(0);
      } else {
      ...
      body: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
        ...
        Expanded(child: LayoutBuilder(builder: (c, cs) { 
          ...
          return GridView.builder(
            ...
            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: cnt, childAspectRatio: ratio, crossAxisSpacing: 16, mainAxisSpacing: 16), 
            itemCount: _filt.length, 
            itemBuilder: (c, i) => InkWell(
              onTap: () => Navigator.push(c, MaterialPageRoute(builder: (_) => DeferredWidget(loadLibrary: details_screen.loadLibrary, builder: () => details_screen.MovieDetailsScreen(movie: _filt[i])))), 
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ClipRRect(borderRadius: BorderRadius.circular(8), child: UniversalImage(path: _filt[i].posterPath)), 
                const SizedBox(height: 8), 
                Text(_filt[i].title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 13))
              ])
            )
          ); 
        }))
