...
class _SearchScreenState extends State<SearchScreen> { 
  ...
  @override void initState() { 
    super.initState(); 
    _filt = RemoteConfigService.searchSuggestions; 
  } 
  
  void _run(String k) {
    setState(() {
      _filt = k.isEmpty 
          ? RemoteConfigService.searchSuggestions 
          : RemoteConfigService.allSnippets.where((e) => e.title.toLowerCase().contains(k.toLowerCase())).toList();
    });
  } 
              ...
              child: LayoutBuilder(builder: (c, cs) { 
                ...
                return GridView.builder(
                  ...
                  itemBuilder: (c, i) => InkWell(
                    onTap: () async { 
                      if(RemoteConfigService.moviesDb.containsKey(_filt[i].id)) { 
                        await details_page.loadLibrary(); 
                        if(context.mounted) {
                          Navigator.push(c, MaterialPageRoute(builder: (_) => details_page.MovieDetailsScreen(movie: RemoteConfigService.moviesDb[_filt[i].id]!))); 
                        }
                      } 
                    }, 
                    child: Column(
                      ...
                      children: [
                        ClipRRect(
                          ...
                          child: Container(color: const Color(0xFF1A1A1A), child: UniversalImage(path: _filt[i].imageUrl))
                        ), 
