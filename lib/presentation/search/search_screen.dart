...
class _S extends State<SearchScreen> { List<MovieModel> _filt = []; @override void initState() { super.initState(); _filt = MockData.allMovies; } void _run(String k) => setState(() => _filt = k.isEmpty ? MockData.allMovies : MockData.allMovies.where((e) => e.title.toLowerCase().contains(k.toLowerCase())).toList()); @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return Scaffold(backgroundColor: AppColors.background, appBar: AppBar(title: Text(p.tr('search')), centerTitle: true, backgroundColor: Colors.transparent, elevation: 0, leading: BackButton(color: Colors.white, onPressed: () { if (p.navIndex != 0) {
  p.setNavIndex(0);
} else {
