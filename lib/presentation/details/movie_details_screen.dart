...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> { 
  ...
  @override void initState() { 
    ...
    _ep = _sea.episodes.isNotEmpty ? _sea.episodes.first : EpisodeModel(id: '0', title: 'No Episode', duration: '', seasonNumber: 0, episodeNumber: 0, sources: []); 
  } 
  ...
  // Task 21: Pure Dynamic Content Note Dialog
  ...
  Widget _buildPlayerInstance() {
    return SmartMediaKitPlayer(
      ...
      subtitles: _ep.subtitles, // Task 23: Map Subtitles
  ...
  @override Widget build(BuildContext context) { 
    ...
    return Scaffold(
      ...
      appBar: _isFullscreen ? null : AppBar(
        ...
        actions: [
          // Task 21: Conditional Logic
          if (widget.movie.infoNote != null && widget.movie.infoNote!.isNotEmpty)
            IconButton(
              icon: widget.movie.infoIconUrl != null 
                  ? UniversalImage(path: widget.movie.infoIconUrl!, width: 24)
  ...
  Widget _info(bool mob, AppProvider p) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    // Task 19: Pure Dynamic Metadata Row
    Row(children: [
      if (widget.movie.rating != null && widget.movie.rating!.isNotEmpty) ...[
        const Icon(Icons.star, color: Colors.amber, size: 18), 
        Text(" ${widget.movie.rating}", style: const TextStyle(color: Colors.greenAccent)), 
        const SizedBox(width: 12), 
      ],
      if (widget.movie.releaseYear != null && widget.movie.releaseYear!.isNotEmpty) ...[
        Text(widget.movie.releaseYear!, style: const TextStyle(color: Colors.white70)), 
        const SizedBox(width: 12), 
      ],
      if (widget.movie.ageRating != null && widget.movie.ageRating!.isNotEmpty)
        Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: AppColors.badgeBg, borderRadius: BorderRadius.circular(4)), child: Text(widget.movie.ageRating!, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)))
    ]), 
