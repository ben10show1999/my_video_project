...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> { late ScrollController _sc; late MovieModel _mov; late SeasonModel _sea; late EpisodeModel _ep; @override void initState() { super.initState(); _sc = ScrollController(); _data(); } 
  ...
  void _data() { 
    // MOCK DATA WITH MULTI-QUALITY & REDUNDANCY
    String url1 = "https://user-images.githubusercontent.com/28053651/202696586-48e7996a-7485-414f-8484-a5416297ea42.mp4";
    String url2 = "https://sample-videos.com/video123/mp4/720/big_buck_bunny_720p_1mb.mp4"; // Backup link
    
    _mov = MovieModel(id: 1, title: "Flutter Mastery", overview: "An epic journey.", posterPath: "", seasons: [
      SeasonModel(id: "s1", title: "Season 1", episodes: List.generate(10, (i) => EpisodeModel(id: "$i", title: "Ep ${i+1}", duration: "24:00", seasonNumber: 1, episodeNumber: i+1, sources: [
        VideoQualityModel(quality: "1080p", urls: [url1, url1]), // 2 links for redundancy
        VideoQualityModel(quality: "720p", urls: [url2]),
        VideoQualityModel(quality: "480p", urls: [url2])
      ])))
    ]); 
  ...
  Widget _desk() => SingleChildScrollView(padding: const EdgeInsets.all(24), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(flex: 3, child: Column(children: [SmartMediaKitPlayer(sources: _ep.sources), const SizedBox(height: 20), _info(false)])), const SizedBox(width: 24), Expanded(flex: 1, child: Column(children: [PlaylistSidebar(isDesktop: true, seasons: _mov.seasons, currentSeason: _sea, currentEpisode: _ep, onSeasonChanged: (s) => setState(() => _sea = s), onEpisodeTap: (e) => setState(() => _ep = e), isLoadingMore: false, onLoadMore: (){}), const SizedBox(height: 24), _over()]))]));
  Widget _info(bool mob) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_mov.title, style: mob ? AppStyles.headlineMedium : AppStyles.headlineLarge), const SizedBox(height: 12), const Row(children: [Icon(Icons.star, color: Colors.amber, size: 18), Text(" 9.8", style: TextStyle(color: Colors.greenAccent))]), const SizedBox(height: 20), SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [ActionIconButton(icon: Icons.add, label: "List", onTap: (){}), const SizedBox(width: 16), ActionIconButton(icon: Icons.share, label: "Share", onTap: _shareContent), const SizedBox(width: 16), ActionIconButton(icon: Icons.flag, label: "Report", onTap: _reportIssue), const SizedBox(width: 16), ActionIconButton(icon: Icons.alarm, label: "Remind", onTap: _remindMe)]))]);
