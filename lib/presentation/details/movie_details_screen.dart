...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> { 
  ...
  void _showInfoDialog(BuildContext context, AppProvider p) { showDialog(context: context, builder: (c) => AlertDialog(backgroundColor: const Color(0xFF202020), title: Center(child: Text(p.tr('content_note'), style: const TextStyle(color: Colors.white))), content: Text(p.tr('content_desc'), style: const TextStyle(color: Colors.white70), textAlign: TextAlign.center), actions: [TextButton(onPressed: () => Navigator.pop(c), child: Text(p.tr('close')))])); }
  ...
  Widget _buildPlayerInstance() {
    return SmartMediaKitPlayer(
      key: ValueKey(_ep.id), // Task 17: Deterministic State reset
      sources: _ep.sources, 
  ...
  @override Widget build(BuildContext context) { 
    ...
    return Scaffold(
      ...
      appBar: _isFullscreen ? null : AppBar(title: Text(p.tr('watch')), centerTitle: true, backgroundColor: Colors.transparent, elevation: 0, leading: const BackButton(color: Colors.white), actions: [IconButton(icon: const Icon(Icons.info_outline, color: Colors.white), onPressed: () => _showInfoDialog(context, p)), const SizedBox(width: 8)]), 
      body: Stack(
        children: [
