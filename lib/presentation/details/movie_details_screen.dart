...
class MovieDetailsScreen extends StatefulWidget { 
  ...
  @override State<MovieDetailsScreen> createState() => _MovieDetailsScreenState(); 
}

class _MovieDetailsScreenState extends State<MovieDetailsScreen> { 
  late ScrollController _sc; 
  late SeasonModel _sea; 
  late EpisodeModel _ep; 
  ...
  void _showInfoDialog(BuildContext context, String note) { 
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        title: const Center(child: Text("ملاحظة المحتوى", style: TextStyle(color: Colors.white))), 
        content: Text(note, style: const TextStyle(color: Colors.white70), textAlign: TextAlign.center), 
  ...
  // Task 36: Global Ad Fallback Injection
  ...
  Widget _mob(AppProvider p) {
    return SingleChildScrollView(
      controller: _sc, 
      child: Column(
        children: [
          if (_isFullscreen) 
            AnimatedContainer(duration: const Duration(milliseconds: 300), curve: Curves.easeInOut, child: AspectRatio(aspectRatio: _currentAspectRatio, child: Container(color: Colors.black))) 
          else 
            AnimatedContainer(duration: const Duration(milliseconds: 300), curve: Curves.easeInOut, child: AspectRatio(aspectRatio: _currentAspectRatio, child: _buildPlayerInstance())), 
            ...
            child: Column(
              ...
              children: [
                ...
                if(widget.movie.seasons.isNotEmpty) 
                  Text(p.tr('episodes'), style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), 
                if(widget.movie.seasons.isNotEmpty) 
                ...
                if(widget.movie.seasons.isNotEmpty) 
                  SizedBox(
                    ...
                    child: PlaylistSidebar(
                      ...
                      currentEpisode: _ep, 
                      adScript: _resolvedAdScript, 
                      onSeasonChanged: (s) => setState(() => _sea = s), 
                      onEpisodeTap: (e) => setState(() => _ep = e), 
                      isLoadingMore: false, 
                      onLoadMore: (){}
                    )
                  ), 
                    ...
                    child: Column(
                      children: [
                        AnimatedContainer(
                          ...
                          child: AspectRatio(
                            aspectRatio: _currentAspectRatio, 
                            child: _isFullscreen ? Container(color: Colors.black) : _buildPlayerInstance()
                          )
                        ), 
                        ...
                        children: [
                          LayoutBuilder(builder: (c, sideConstraints) { 
                            ...
                            return AnimatedContainer(
                              duration: const Duration(milliseconds: 400), 
                              curve: Curves.fastOutSlowIn, 
                              height: dynamicHeight.clamp(300.0, 800.0), 
                              child: PlaylistSidebar(
                                isDesktop: true, 
                                seasons: widget.movie.seasons, 
                                currentSeason: _sea, 
  ...
  Widget _info(bool mob, AppProvider p) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start, 
      children: [
        ...
        Row(
          children: [
            if (widget.movie.rating != null && widget.movie.rating!.isNotEmpty) 
              ...[const Icon(Icons.star, color: Colors.amber, size: 18), Text(" ${widget.movie.rating}", style: const TextStyle(color: Colors.greenAccent)), const SizedBox(width: 12)], 
            if (widget.movie.releaseYear != null && widget.movie.releaseYear!.isNotEmpty) 
              ...[Text(widget.movie.releaseYear!, style: const TextStyle(color: Colors.white70)), const SizedBox(width: 12)], 
