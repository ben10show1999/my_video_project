...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> { 
  ...
  // Task 1: Elastic Aspect Ratio State
  ...
  // Task 1: Update Layout when video size changes
  ...
  @override Widget build(BuildContext context) { 
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text(p.tr('watch')), centerTitle: true, backgroundColor: Colors.transparent, elevation: 0, leading: const BackButton(color: Colors.white), actions: [IconButton(icon: const Icon(Icons.info_outline, color: Colors.white), onPressed: () => _showInfoDialog(context, p)), const SizedBox(width: 8)]),
  ...
  // Task 2: Symbiotic Layout Logic (Inverse Width Calculation)
  Widget _desk(AppProvider p) { return LayoutBuilder(builder: (context, constraints) { 
    double availableWidth = constraints.maxWidth - 48; 
    // Calculate shares based on ratio (Wide = more player width, Square = less player width)
    double playerWidthShare = _currentAspectRatio >= 1.7 ? 0.68 : (_currentAspectRatio < 1.0 ? 0.40 : 0.55);
