...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> { 
  ...
  @override void initState() { super.initState(); _sc = ScrollController(); _sea = widget.movie.seasons.isNotEmpty ? widget.movie.seasons.first : SeasonModel(id: '0', title: 'No Seasons', episodes: []); _ep = _sea.episodes.isNotEmpty ? _sea.episodes.first : EpisodeModel(id: '0', title: 'No Episode', duration: '', seasonNumber: 0, episodeNumber: 0, sources: [], subtitles: {}); } 
  ...
  Widget _buildPlayerInstance() {
    return SmartMediaKitPlayer(
      key: ValueKey(_ep.id), // Task 17: Deterministic Isolation
      sources: _ep.sources, 
      ...
      subtitles: _ep.subtitles, // Task 20: Map passed
      originalLang: _ep.originalLang,
  ...
  @override Widget build(BuildContext context) { 
    ...
    return Scaffold(
      ...
      appBar: _isFullscreen ? null : AppBar(
        ...
        actions: [
          // Task 19 Purge: Strictly conditional info icon based on config.json
          if (widget.movie.contentNoteText != null && widget.movie.contentNoteText!.isNotEmpty)
            IconButton(
              icon: widget.movie.contentNoteIconUrl != null 
                ? SizedBox(width: 24, height: 24, child: UniversalImage(path: widget.movie.contentNoteIconUrl!, fit: BoxFit.contain))
                : const Icon(Icons.info_outline, color: Colors.white), 
              onPressed: () => _showInfoDialog(context, p)
            ), 
  ...
  Widget _info(bool mob, AppProvider p) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    // Task 19 Purge: Strictly conditional metadata row
    Row(children: [
