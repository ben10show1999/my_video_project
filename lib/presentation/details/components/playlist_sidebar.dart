...
class PlaylistSidebar extends StatefulWidget { 
  ...
  const PlaylistSidebar({
    ...
    required this.isLoadingMore, required this.onLoadMore, this.adScript
  }); 
...
class _PlaylistSidebarState extends State<PlaylistSidebar> { 
  ...
  @override void initState() { 
    super.initState(); 
    if(widget.adScript == null) _ad = false; 
            ...
            child: Column(
              children: [
                DropdownButton<SeasonModel>(
                  ...
                  isExpanded: true, 
                  items: widget.seasons.map((s) => DropdownMenuItem(value: s, child: Text(s.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white)))).toList(), 
                  onChanged: (v) { if (v != null) { widget.onSeasonChanged(v); } }
                ), 
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween, 
                  children: [
                    Text(p.tr('autoplay'), style: const TextStyle(color: Colors.grey)), 
                    Switch(value: p.isAutoplayEnabled, onChanged: (v) => p.toggleAutoplay(v), activeTrackColor: Colors.amber.withValues(alpha: 0.5), activeThumbColor: Colors.amber)
                  ]
                )
              ]
            )
  ...
  Widget _list() {
    return ListView.builder(
      ...
      itemBuilder: (c, i) { 
        if (_ad && i == 0) { 
          return WebAdBanner(adScript: widget.adScript, onClose: () => setState(() => _ad = false)); 
        } 
        final epIndex = _ad ? i - 1 : i; 
        if (epIndex == widget.currentSeason.episodes.length) { 
          return const Center(child: CircularProgressIndicator(color: Colors.amber)); 
        } 
        final e = widget.currentSeason.episodes[epIndex]; 
        final sel = e.id == widget.currentEpisode.id; 
        return InkWell(
          onTap: () => widget.onEpisodeTap(e), 
          child: Container(
            padding: const EdgeInsets.all(8), 
            color: sel ? Colors.white10 : Colors.transparent, 
            child: Row(
              children: [
