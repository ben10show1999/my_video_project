...
class PlaylistSidebar extends StatefulWidget { 
  ...
  final bool isDesktop; final bool isLoadingMore; final VoidCallback onLoadMore; final String? adScript;
  const PlaylistSidebar({super.key, required this.seasons, required this.currentSeason, required this.currentEpisode, required this.onSeasonChanged, required this.onEpisodeTap, this.isDesktop = false, required this.isLoadingMore, required this.onLoadMore, this.adScript}); 
...
class _PlaylistSidebarState extends State<PlaylistSidebar> { 
  ...
  Widget _list() => ListView.builder(
    ...
    itemCount: widget.currentSeason.episodes.length + (_ad ? 1 : 0) + (widget.isLoadingMore ? 1 : 0), 
    itemBuilder: (c, i) { 
      if (_ad && i == 0) { return WebAdBanner(adScript: widget.adScript, onClose: () => setState(() => _ad = false)); }
      ...
      return InkWell(onTap: () => widget.onEpisodeTap(e), child: Container(padding: const EdgeInsets.all(8), color: sel ? Colors.white10 : Colors.transparent, child: Row(children: [
        SizedBox(width: 100, height: 60, child: ClipRRect(borderRadius: BorderRadius.circular(4), child: UniversalImage(path: e.thumbnailUrl ?? "https://iili.io/fZAinLv.webp", fit: BoxFit.cover))), 
