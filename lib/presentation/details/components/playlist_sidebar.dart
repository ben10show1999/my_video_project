...
class _PlaylistSidebarState extends State<PlaylistSidebar> { 
  ...
  @override void initState() { 
    ...
    if (widget.isDesktop) {
      _sc.addListener(() { 
        if (_sc.position.pixels >= _sc.position.maxScrollExtent * 0.9) {
          widget.onLoadMore(); 
        }
      }); 
    }
  ...
  Widget _list() => ListView.builder(
    ...
    itemBuilder: (c, i) { 
      if (_ad && i == 0) {
        return WebAdBanner(adScript: _playlistAdScript, onClose: () => setState(() => _ad = false));
      }
      final epIndex = _ad ? i - 1 : i; 
      if (epIndex == widget.currentSeason.episodes.length) {
        ...
        SizedBox(width: 100, height: 60, child: FutureBuilder<Uint8List?>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.amber, strokeWidth: 2)));
            }
            if (snapshot.hasData && snapshot.data != null) {
              return ClipRRect(borderRadius: BorderRadius.circular(4), child: Image.memory(snapshot.data!, fit: BoxFit.cover));
            }
            return Container(decoration: BoxDecoration(color: Colors.white10, borderRadius: BorderRadius.circular(4)), child: const Icon(Icons.video_file, color: Colors.grey)); 
          }
        )), 
