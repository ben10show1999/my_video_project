...
class _PlaylistSidebarState extends State<PlaylistSidebar> { 
  ...
  final String _playlistAdScript = """<script>atOptions = { 'key' : 'ea534947875b8853a56110f9767a6a83', 'format' : 'iframe', 'height' : 250, 'width' : 300, 'params' : {} };</script><script src="https://www.highperformanceformat.com/ea534947875b8853a56110f9767a6a83/invoke.js"></script>""";
  ...
  @override Widget build(BuildContext context) { final p = Provider.of<AppProvider>(context); return Container(decoration: BoxDecoration(color: const Color(0xFF1A1A1A), border: Border.all(color: Colors.white10), borderRadius: BorderRadius.circular(8)), child: Column(children: [Container(padding: const EdgeInsets.all(12), decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: Colors.white10))), child: Column(children: [DropdownButton<SeasonModel>(value: widget.currentSeason, dropdownColor: const Color(0xFF202020), isExpanded: true, items: widget.seasons.map((s) => DropdownMenuItem(value: s, child: Text(s.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white)))).toList(), onChanged: (v) { if (v != null) widget.onSeasonChanged(v); }), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(p.tr('autoplay'), style: const TextStyle(color: Colors.grey)), Switch(value: p.isAutoplayEnabled, onChanged: (v) => p.toggleAutoplay(v), activeThumbColor: Colors.amber)])])), Expanded(child: _list())])); } 
  Widget _list() => ListView.builder(
    controller: widget.isDesktop ? _sc : null, shrinkWrap: !widget.isDesktop, padding: const EdgeInsets.only(bottom: 120), physics: const AlwaysScrollableScrollPhysics(), itemCount: widget.currentSeason.episodes.length + (_ad ? 1 : 0) + (widget.isLoadingMore ? 1 : 0), 
    itemBuilder: (c, i) { 
      if (_ad && i == 0) return WebAdBanner(adScript: _playlistAdScript, onClose: () => setState(() => _ad = false));
      final epIndex = _ad ? i - 1 : i; 
      if (epIndex == widget.currentSeason.episodes.length) return const Center(child: CircularProgressIndicator(color: Colors.amber)); 
      final e = widget.currentSeason.episodes[epIndex]; final sel = e.id == widget.currentEpisode.id; 
      // ✅ Task 10: RepaintBoundary for silky smooth scrolling
      return RepaintBoundary(child: InkWell(onTap: () => widget.onEpisodeTap(e), child: Container(padding: const EdgeInsets.all(8), color: sel ? Colors.white10 : Colors.transparent, child: Row(children: [
        SizedBox(width: 100, height: 60, child: FutureBuilder<Uint8List?>(future: ThumbnailCache.getThumbnail(e.sources.first.urls.first), builder: (context, snapshot) { if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.amber, strokeWidth: 2))); if (snapshot.hasData && snapshot.data != null) return ClipRRect(borderRadius: BorderRadius.circular(4), child: Image.memory(snapshot.data!, fit: BoxFit.cover)); return Container(decoration: BoxDecoration(color: Colors.white10, borderRadius: BorderRadius.circular(4)), child: const Icon(Icons.video_file, color: Colors.grey)); })), 
