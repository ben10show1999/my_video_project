...
class _LazyHorizontalSectionState extends State<LazyHorizontalSection> { 
  ...
  @override void initState() { 
    super.initState(); _m = List.from(widget.initialMovies); 
    _sc.addListener(() { if (_sc.position.pixels >= _sc.position.maxScrollExtent * 0.8 && !_load && _more) { _fetch(); } }); 
  } 
  
  Future<void> _fetch() async { 
    setState(() { _load = true; }); 
    final n = await widget.onLoadMore(); 
    if (mounted) { setState(() { if (n.isEmpty) { _more = false; } else { _m.addAll(n); } _load = false; }); } 
  } 
