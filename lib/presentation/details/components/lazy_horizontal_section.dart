...
class LazyHorizontalSection extends StatefulWidget { 
  ...
  final List<MovieSnippet> initialMovies; 
  final Future<List<MovieSnippet>> Function() onLoadMore; 
...
class _LazyHorizontalSectionState extends State<LazyHorizontalSection> { 
  ...
  @override void initState() { 
    super.initState(); 
    _m = List.from(widget.initialMovies); 
    _sc.addListener(() { 
  ...
  Future<void> _fetch() async { 
    setState(() { _load = true; }); 
    final n = await widget.onLoadMore(); 
    if (mounted) { 
      setState(() { 
        if (n.isEmpty) { _more = false; } else { _m.addAll(n); } 
        _load = false; 
      }); 
    } 
  } 
    ...
    return Column(
      ...
      children: [
        Padding(
          ...
          child: Row(
            children: [
              ...
              TextButton(
                onPressed: () async { 
                  await view_all_page.loadLibrary(); 
                  if(context.mounted) Navigator.push(context, MaterialPageRoute(builder: (_) => view_all_page.ViewAllScreen(title: widget.title, showTitles: widget.showTitles, movies: _m))); 
                }, 
                child: Text(p.tr('view_all'))
              )
            ]
          )
        ), 
        ...
        LayoutBuilder(builder: (c, cs) { 
          ...
          double itemHeight = widget.showTitles ? ((itemWidth * 1.5) + 65) : (itemWidth * 1.5); 
                  ...
                  child: SizedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if(widget.showTitles) const SizedBox(height: 6), 
                        if(widget.showTitles) Text(m.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white70, fontSize: 12))
                      ]
                    )
                  )
                ); 
