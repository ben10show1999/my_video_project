...
// �7�3 FIXED: Using absolute package imports to prevent path errors
...
class _LazyHorizontalSectionState extends State<LazyHorizontalSection> { 
  ...
  void initState() { 
    ...
    _sc.addListener(() { 
      if (_sc.position.pixels >= _sc.position.maxScrollExtent * 0.8 && !_load && _more) _fetch(); 
    }); 
  } 
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Padding(
        ...
        child: Row(children: [
          ...
          const Spacer(), 
          TextButton(
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ViewAllScreen(title: widget.title, movies: _m))), 
      ...
      LayoutBuilder(builder: (c, cs) {
        ...
        return SizedBox(height: itemHeight + 35, child: ListView.separated(
          controller: _sc, 
          ...
          itemBuilder: (c, i) { 
            ...
            return InkWell(
              onTap: () {
                 // �7�3 FIXED: Correctly finds the movie from MockData
                 final targetMovie = MockData.allMovies.firstWhere(
                   (e) => e.id.toString() == m.id, 
                   orElse: () => MockData.allMovies.first
                 );
                 Navigator.push(context, MaterialPageRoute(builder: (_) => MovieDetailsScreen(movie: targetMovie)));
              }, 
              child: SizedBox(width: itemWidth, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
