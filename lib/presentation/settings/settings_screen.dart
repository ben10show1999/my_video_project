...
class SettingsScreen extends StatelessWidget { 
  ...
  void _showAboutApp(BuildContext context, AppProvider p) { showDialog(context: context, builder: (c) => AlertDialog(backgroundColor: const Color(0xFF202020), title: Center(child: Text(p.tr('about_app'), style: const TextStyle(color: Colors.white))), content: const Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.play_circle_fill, size: 60, color: AppColors.primary), SizedBox(height: 16), Text("Version 7.0.0", style: TextStyle(color: Colors.white70)), Text("Immortal Build (Zero Jank & Fluid)", style: TextStyle(color: Colors.grey, fontSize: 12))]), actions: [TextButton(onPressed: () => Navigator.pop(c), child: Text(p.tr('close')))])); } 
    ...
    return Scaffold(
      ...
      body: ListView(children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), child: Row(children: [
          const Icon(Icons.language, color: Colors.white), const SizedBox(width: 16), Expanded(child: Text(p.tr('language'), style: const TextStyle(color: Colors.white, fontSize: 16))), 
          DropdownButton<String>(dropdownColor: const Color(0xFF202020), value: p.currentLanguageCode, iconEnabledColor: Colors.white, items: ['auto', 'en', 'ar'].map((code) => DropdownMenuItem(value: code, child: Text(p.tr(code), style: const TextStyle(color: Colors.white)))).toList(), onChanged: (v) { if (v != null) { p.setLanguage(v); } }, underline: Container())
        ])), 
