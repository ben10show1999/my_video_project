...
class SettingsScreen extends StatelessWidget { const SettingsScreen({super.key}); 
  ...
  @override Widget build(BuildContext context) { 
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text(p.tr('settings')), centerTitle: true, leading: const BackButton(color: Colors.white), backgroundColor: Colors.transparent, elevation: 0),
      body: ListView(children: [
        ...
        ListTile(
          title: Text(p.tr('language'), style: const TextStyle(color: Colors.white)),
          leading: const Icon(Icons.language, color: Colors.white),
          trailing: DropdownButton<String>(dropdownColor: const Color(0xFF202020), value: p.currentLanguageCode, items: ['auto', 'en', 'ar'].map((code) => DropdownMenuItem(value: code, child: Text(p.tr(code), style: const TextStyle(color: Colors.white)))).toList(), onChanged: (v) { if(v != null) p.setLanguage(v); }, underline: Container()),
        ),
