...
class SettingsScreen extends StatelessWidget { 
  ...
  void _showAboutApp(BuildContext context, AppProvider p) { showDialog(context: context, builder: (c) => AlertDialog(backgroundColor: const Color(0xFF202020), title: Center(child: Text(p.tr('about_app'), style: const TextStyle(color: Colors.white))), content: const Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.play_circle_fill, size: 60, color: AppColors.primary), SizedBox(height: 16), Text("Version 5.0.0", style: TextStyle(color: Colors.white70)), Text("Master Build (100% Purged)", style: TextStyle(color: Colors.grey, fontSize: 12))]), actions: [TextButton(onPressed: () => Navigator.pop(c), child: Text(p.tr('close')))])); } 
