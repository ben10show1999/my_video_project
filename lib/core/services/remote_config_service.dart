...
class RemoteConfigService {
  // 💡 الهدف تم تحديثه إلى المستودع الجديد
  ...
  static Future<bool> fetchConfig({bool forceRefresh = false}) async {
    ...
    try {
      // 💡 الحل الجذري: كسر الكاش وتخطي حماية CORS عبر إضافة ختم زمني
      // بدون إرسال ترويسات مخصصة لتجنب فشل Preflight OPTIONS
      final String cacheBuster =
          DateTime.now().millisecondsSinceEpoch.toString();
      final Uri targetUri = Uri.parse("$_githubUrl?v=$cacheBuster");

      final response =
          await http.get(targetUri).timeout(const Duration(seconds: 15));
