...
class UniversalImage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    // ENFORCED 2:3 RATIO TO PREVENT STRETCHING/GAPS
    return AspectRatio(
      aspectRatio: 2 / 3,
      child: _buildImage(),
    );
  ...
  Widget _buildImage() {
    if (path.startsWith('http')) {
      return CachedNetworkImage(
        ...
        placeholder: (c, u) => Container(color: AppColors.surface, child: const Center(child: CircularProgressIndicator(color: AppColors.primary, strokeWidth: 2))),
  ...
  Widget _buildErrorWidget() {
    ...
    return Container(
      ...
      child: LayoutBuilder(builder: (context, constraints) {
        return Center(child: Icon(Icons.broken_image_rounded, color: Colors.grey, size: constraints.maxWidth * 0.3));
      }),
    );
  }
}
