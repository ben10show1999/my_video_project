...
class ThumbnailCache {
  ...
  static Future<Uint8List?> _extractFrame(String videoUrl) {
    ...
    try {
      ...
      video.setAttribute('disableRemotePlayback', 'true'); video.src = videoUrl;
      ...
      video.load();
      // ✅ Strict 2-second timeout to kill Ghost Loading
      Future.delayed(const Duration(seconds: 2), () { if (!completer.isCompleted) { cleanup(); completer.completeError('Timeout'); } });
    } catch (e) { cleanup(); if (!completer.isCompleted) completer.completeError(e); }
    return completer.future;
  }
}
