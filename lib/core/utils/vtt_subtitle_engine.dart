...
class VttSubtitleEngine {
  ...
  static String? currentSubEn;
  static String? currentSubAr;

  static Future<void> loadSubtitles(String requestedLang, {String? enUrl, String? arUrl}) async {
    currentSubEn = enUrl;
    currentSubAr = arUrl;
    ...
    if (requestedLang == "Auto") {
      ...
      if (sysLang == 'ar' && arUrl != null) { url = arUrl; } 
      else if (enUrl != null) { url = enUrl; }
    } else if (requestedLang == "Arabic" && arUrl != null) {
      url = arUrl;
    } else if (requestedLang == "English" && enUrl != null) {
      url = enUrl;
    }
    ...
    try {
      // Basic Decryption if encrypted in JSON
  ...
  static void updatePosition(Duration position) {
    ...
    for (var cue in _cues) {
      if (position >= cue.start && position <= cue.end) {
        if (currentSubtitle.value != cue.text) {
