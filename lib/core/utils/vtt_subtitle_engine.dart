...
class VttSubtitleEngine {
  ...
  // Task 20: Cognitive Subtitle Engine Logic
  static Future<void> loadSubtitles(String requestedLang, Map<String, String> availableSubs, String? originalLang) async {
    String url = "";
    
    if (requestedLang == "Auto") {
      final sysLang = PlatformDispatcher.instance.locale.languageCode;
      if (availableSubs.containsKey(sysLang)) {
        url = availableSubs[sysLang]!;
      } else if (originalLang != null && availableSubs.containsKey(originalLang)) {
        url = availableSubs[originalLang]!;
      }
    } else if (availableSubs.containsKey(requestedLang)) {
      url = availableSubs[requestedLang]!;
    }
