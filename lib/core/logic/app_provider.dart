...
class AppProvider extends ChangeNotifier {
  ...
  final List<AppNotification> _notifications = []; // ✅ Fixed: added final
  ...
  Locale get locale {
    if (_languageCode == 'auto') {
      ...
      if (systemLoc == 'ar') { return const Locale('ar'); } else { return const Locale('en'); } // ✅ Fixed
    }
  ...
  List<AppNotification> get notifications {
    if (_notificationsEnabled) { return _notifications; } else { return []; } // ✅ Fixed
  }
  ...
  void toggleMyList(int id) {
    if (_myListIds.contains(id)) { _myListIds.remove(id); } else { _myListIds.add(id); } // ✅ Fixed
    notifyListeners();
  }
