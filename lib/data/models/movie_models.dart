class AppNotificationModel {
  ...
  AppNotificationModel({
    ...
    this.imageUrl, this.targetId, this.timestamp
  });
...
class EpisodeModel {
  ...
  EpisodeModel({
    required this.id, required this.title, this.thumbnailUrl, required this.duration, 
    required this.seasonNumber, required this.episodeNumber, required this.sources, required this.subtitles
  });
...
class MovieModel { 
  ...
  final List<SeasonModel> seasons; final String? adScript; final String? rating; 
  final String? releaseYear; final String? ageRating; final String? infoIconUrl; final String? infoNote;
  
  const MovieModel({
    required this.id, required this.title, required this.overview, required this.posterPath, 
    this.seasons = const [], this.adScript, this.rating, this.releaseYear, this.ageRating, 
    this.infoIconUrl, this.infoNote
  }); 
  
  factory MovieModel.fromJson(Map<String, dynamic> json) {
    return MovieModel(
      id: json['id'] ?? '', title: json['title'] ?? '', overview: json['overview'] ?? '', 
      posterPath: json['posterPath'] ?? '', adScript: json['adScript'], rating: json['rating'], 
      releaseYear: json['releaseYear'], ageRating: json['ageRating'], 
      infoIconUrl: json['infoIconUrl'], infoNote: json['infoNote'], 
      seasons: (json['seasons'] as List?)?.map((e) => SeasonModel.fromJson(e)).toList() ?? []
    );
  }
}
