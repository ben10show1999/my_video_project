...
class EpisodeModel {
  ...
  final int seasonNumber; final int episodeNumber; final List<VideoQualityModel> sources; final Map<String, String> subtitles; 
  ...
  factory EpisodeModel.fromJson(Map<String, dynamic> json) => EpisodeModel(id: json['id'] ?? '', title: json['title'] ?? '', thumbnailUrl: json['thumbnailUrl'], duration: json['duration'] ?? '', seasonNumber: json['seasonNumber'] ?? 1, episodeNumber: json['episodeNumber'] ?? 1, sources: (json['sources'] as List?)?.map((e) => VideoQualityModel.fromJson(e)).toList() ?? [], subtitles: Map<String, String>.from(json['subtitles'] ?? {}));
}
...
class MovieModel { 
  ...
  final List<SeasonModel> seasons; final String? adScript; final String? rating; final String? releaseYear; final String? ageRating; final String? infoIconUrl; final String? infoNote;
...
// Task 41: Cloud-Sync Notification Model
class AppNotification {
  final String id; final String title; final String body; final String time; 
  final String? targetId; final String? imageUrl;
  AppNotification({required this.id, required this.title, required this.body, required this.time, this.targetId, this.imageUrl});
  factory AppNotification.fromJson(Map<String, dynamic> json) => AppNotification(id: json['id'] ?? DateTime.now().millisecondsSinceEpoch.toString(), title: json['title'] ?? '', body: json['body'] ?? '', time: json['time'] ?? 'Just Now', targetId: json['targetId'], imageUrl: json['imageUrl']);
}
