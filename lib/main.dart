...
void main() async { 
  ...
  try {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async { await RemoteConfigService.fetchConfig(forceRefresh: true); });
...
class MyApp extends StatelessWidget { 
  ...
  @override Widget build(BuildContext context) { 
    ...
    return MaterialApp(
      ...
      title: 'Video Project Engine V9', 
      locale: p.locale,
      supportedLocales: const [Locale('en'), Locale('ar')],
      localizationsDelegates: const [GlobalMaterialLocalizations.delegate, GlobalWidgetsLocalizations.delegate, GlobalCupertinoLocalizations.delegate],
      theme: ThemeData.dark().copyWith(scaffoldBackgroundColor: Colors.black, colorScheme: const ColorScheme.dark(primary: Colors.amber), textTheme: GoogleFonts.cairoTextTheme(ThemeData.dark().textTheme), appBarTheme: const AppBarTheme(backgroundColor: Colors.transparent, elevation: 0)), 
      home: FutureBuilder(future: app_shell.loadLibrary(), builder: (c, s) => s.connectionState == ConnectionState.done ? app_shell.MainScreen() : const Scaffold(backgroundColor: Colors.black))
    ); 
  } 
}
